#!/usr/bin/env python3
# HH_260310-00:00 Module-level status_stream publisher for planning outputs.
...
class PlanningStatusNode(Node):
    # Implements `__init__` behavior.
    def __init__(self) -> None:
        super().__init__("planning_status")
        ...
        self.get_logger().info(
            "planning_status: status=%s period=%.2fs"
    ...
    def _state(self, now: Time) -> ModuleState:
        ...
        if goal_seen:
            ...
            if missing:
                ...
                out.message = "planning outputs statey"
        else:
...
def main() -> None:
    rclpy.init()
    node = PlanningStatusNode()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    except Exception as e:  # noqa: BLE001
        node.get_logger().error(f"planning_status runtime exception: {e}")
    finally:
        node.destroy_node()
        try:
            if rclpy.ok():
