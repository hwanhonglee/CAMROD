...
LifecycleManager::changeStateForNode(const std::string & node_name, std::uint8_t transition)
{
  ...
  if (!node_map_[node_name]->change_state(transition)) {
    RCLCPP_ERROR(get_logger(), "Failed to request transition for node: %s", node_name.c_str());
    return false;
  }

  // HH_260317-00:00 Lifecycle transition guard:
  ...
  while (observed_state != expected_state && std::chrono::steady_clock::now() < deadline) {
    ...
    observed_state = node_map_[node_name]->get_state();
  }

  if (observed_state != expected_state) {
    RCLCPP_ERROR(
      get_logger(),
      "Failed to change state for node: %s (expected=%u observed=%u)",
      node_name.c_str(),
      static_cast<unsigned int>(expected_state),
      static_cast<unsigned int>(observed_state));
    return false;
  }
