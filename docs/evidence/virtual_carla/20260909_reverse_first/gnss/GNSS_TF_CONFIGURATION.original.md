...
설정 근거는 [Ranger 센서 spawn JSON](/home/hong/camrod_ws/src/camrod_carla_adapter/config/ranger_spawn_camrod_full_sensors.json:221), [sensor kit 장착 YAML](/home/hong/camrod_ws/src/camrod_sensor_kit/config/robot_params.yaml:70)이다. 왼쪽 GNSS가 localization 주 입력이며 오른쪽은 호환 센서 정보에 사용된다. spawn의 GNSS tick은 0.1s, noise/bias는 0이다. 이는 **노이즈 없는 시뮬레이션 센서 설정**이지 실제 RTK 정확도·위성 수·전파환경 검증이 아니다.
...
핵심 구현은 [onNavSatFix](/home/hong/camrod_ws/src/camrod_localization/src/localization_input_adapter_node.cpp:549)와 [antennaPositionToRobotCenter](/home/hong/camrod_ws/src/camrod_localization/include/camrod_localization/gnss_lever_arm.hpp:17)이다. 안테나 body offset을 `(a,b)`, 방향을 `ψ`라 하면:
...
1. 표준 CARLA ROS bridge가 Unreal의 좌수계 좌표를 ROS 우수계로 바꾼다: 위치 `(x,y,z)→(x,−y,z)`, yaw 부호 반전. [실제 bridge 변환 코드](/home/hong/Downloads/ranger-carla-4ws-pipeline/.work/ros-bridge-ws/src/ros-bridge/carla_common/src/carla_common/transforms.py:81).
2. feedback bridge는 CARLA ROS 위치·자세를 CAMROD map과 정렬한다. 실행 값은 translation `(6.9521841297,9.4901857126)m`, yaw `8.14281112e−7rad`이다. 이 map 정렬은 센서 0.45m 보정과 별개다. raw GNSS 위·경도에는 이 평면 translation을 직접 더하지 않는다. [정렬 설정](/home/hong/camrod_ws/src/camrod_carla_adapter/config/woraksan_lane_anchor_alignment.yaml:16), [실제 IMU 변환](/home/hong/camrod_ws/src/camrod_carla_adapter/src/camrod_carla_adapter/feedback_bridge_node.py:405).
...
4. production input_adapter가 그 heading에 배포 trim **−92°**를 적용한다. [onGnssHeading](/home/hong/camrod_ws/src/camrod_localization/src/localization_input_adapter_node.cpp:488), [입력 설정](/home/hong/camrod_ws/src/camrod_localization/config/source/input_adapter.yaml:65).

현재 순효과는 map 정렬된 CARLA IMU yaw 대비 **−2°**다. [feedback YAML](/home/hong/camrod_ws/src/camrod_carla_adapter/config/feedback_bridge.yaml:38) 및 bridge 주석에 남은 “unchanged −90°”는 현재 −92° 배포값과 맞지 않는 과거 설명이다. **이번 감사에서 −92를 −90으로 바꾸지 않았다.** 이 trim의 실차 교정 근거까지 새로 검증한 것은 아니다.
...
sensor kit의 고정 TF는 [sensor_kit.launch.py](/home/hong/camrod_ws/src/camrod_sensor_kit/launch/sensor_kit.launch.py:158)가 YAML을 xacro 인자로 넘겨 `/sensor_kit/robot_state_publisher`에서 발행한다. [xacro 중심/뒤축 연결](/home/hong/camrod_ws/src/camrod_sensor_kit/urdf/camrod_sensor_kit.xacro:209), [GNSS joint](/home/hong/camrod_ws/src/camrod_sensor_kit/urdf/camrod_sensor_kit.xacro:240), [map→odom 고정 publisher](/home/hong/camrod_ws/src/camrod_localization/launch/localization.launch.py:116).
...
실제 EKF는 20Hz, `two_d_mode=true`, `world_frame=odom`, `base_link_frame=robot_center_link`, `publish_tf=true`다. 같은 GNSS pose를 `pose0`에서는 XYZ, `pose1`에서는 yaw만 선택해 사용하며 같은 XYZ를 이중 융합하지 않는다. IMU에서는 roll/pitch·각속도를, wheel 입력에서는 vx/vy/yaw-rate를 사용한다. [EKF 설정](/home/hong/camrod_ws/src/camrod_localization/config/filter/ekf.yaml:16).
...
예전 [metric-truth 전용 입력 YAML](/home/hong/camrod_ws/src/camrod_carla_adapter/config/camrod_input_adapter_carla.yaml:5)은 GNSS lever arm이 꺼져 있지만 **현재 활성 설정이 아니다.** 현재 full launch는 production 입력 YAML을 사용하며, 실제 `utm_pose_topic=''`와 NavSatFix 입력·보정 ON으로 재확인했다. [launch 기본값](/home/hong/camrod_ws/src/camrod_carla_adapter/launch/camrod_carla_full.launch.py:96), [bringup 전달](/home/hong/camrod_ws/src/camrod_carla_adapter/launch/camrod_carla_full.launch.py:1107).
...
- TF 장착 위치: [camrod_sensor_kit/config/robot_params.yaml의 gnss](/home/hong/camrod_ws/src/camrod_sensor_kit/config/robot_params.yaml:70). 실제 운영 launch에서 이 파일 또는 별도 profile을 선택하는지 확인한다.
- planar 데이터 보정: 실제 선택된 [localization 입력 YAML](/home/hong/camrod_ws/src/camrod_localization/config/source/input_adapter.yaml:23)의 `gnss_antenna_offset_x_m/y_m`. [bringup 쪽 대응 설정](/home/hong/camrod_ws/src/camrod_bringup/config/localization/source/input_adapter.yaml:23)도 존재하므로 파일 한쪽만 보고 운영값을 단정하지 말고 **실제 노드 파라미터**와 일치시킨다.
...
이미 확보한 [−90.416908° 부분구간 분석](gnss_rotation_observation_02/first90_of_recorded_turn/README.md)은 실제 −178.176376° 연속 회전의 첫 부분이다. raw 안테나 이동 약 62.44cm, 실제 중심 이동 4.18cm, 보정 직후 중심 이동 3.19cm, 보정−truth 오차벡터 변화 1.35cm를 구분해 기록했다. **90° 명령을 내리고 정지한 결과가 아니며**, 180° 명령 후 정착 오차까지 새로 검증한 보고서도 아니다.
