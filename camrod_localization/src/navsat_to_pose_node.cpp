...
// Implements `rotatePointXY` behavior.
avg_msgs::msg::Point rotatePointXY(
  const avg_msgs::msg::Point & in, const double yaw_rad)
{
...
private:
  ...
  // Handles the `onNavSatFix` callback.
  ...
  // Handles the `onRawPose` callback.
  ...
  // Handles the `onUtmPose` callback.
  ...
  // Implements `deduceType` behavior.
  InputType deduceType(const avg_msgs::msg::PoseStamped & msg) const
  {
  ...
  // Implements `convertLatLon` behavior.
  avg_msgs::msg::Point convertLatLon(double lat_deg, double lon_deg, double alt) const
  {
  ...
  // Implements `yawFromMsg` behavior.
  double yawFromMsg(const avg_msgs::msg::PoseStamped & msg) const
  {
    ...
    const rclcpp::Time & stamp, const avg_msgs::msg::Point & enu, double yaw) const
  {
    ...
    if (publish_covariance_ && pose_cov_pub_) {
      avg_msgs::msg::PoseWithCovarianceStamped cov_msg;
    ...
    if (publish_localization_diagnostic_ && avg_localization_pub_) {
      ...
      avg_msg.gnss_pose_cov.pose.pose = out.pose;
      std::copy(covariance_.begin(), covariance_.end(), avg_msg.gnss_pose_cov.pose.covariance.begin());
      if (has_navsatfix_) {
  ...
  avg_msgs::msg::NavSatFix last_navsatfix_{};
};
}  // namespace camping_cart::localization
