...
class NTRIPRosBase(Node):
    def __init__(self, name):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                # CHANGED: default to rtcm_msgs, not mavros_msgs
        ...
        if rtcm_message_package == _MAVROS_MSGS_NAME:
            if have_mavros_msgs:
                ...
                self.get_logger().fatal(
                    f'The requested RTCM package {rtcm_message_package} is a valid option, '
                    'but we were unable to import it. Please make sure you have it installed'
                )
                raise RuntimeError(
                    f'Failed to import RTCM message package: {rtcm_message_package}'
                )
            ...
            self.get_logger().fatal(
                f'The RTCM package {rtcm_message_package} is not a valid option. '
                f'Please choose between the following packages '
                f'{",".join([_MAVROS_MSGS_NAME, _RTCM_MSGS_NAME])}'
            )
            raise RuntimeError(
                f'Invalid RTCM package option: {rtcm_message_package}'
            )
    ...
    def subscribe_fix(self, fix: NavSatFix):
        ...
        minute = t.minute
        second = t.second
        millisecond = int(t.microsecond * 1e-4)
        ...
        nmea_sentence_no_checksum = (
            f"$GPGGA,{nmea_utc},{nmea_lat},{nmea_lat_direction},"
            f"{nmea_lon},{nmea_lon_direction},{nmea_status},05,1.0,100.0,M,-32.0,M,,0000"
        )
