#!/usr/bin/env python3
# HH_260310-00:00 Module-level status_stream publisher for sensing outputs.
...
class SensingStatusNode(Node):
    # Implements `__init__` behavior.
    def __init__(self) -> None:
        super().__init__("sensing_status")
        ...
        self.get_logger().info(
            "sensing_status: status=%s period=%.2fs"
    ...
    def _state(self, now: Time) -> ModuleState:
        ...
        if missing:
            ...
            out.message = "sensing streams statey"
        return out
...
def main() -> None:
    rclpy.init()
    node = SensingStatusNode()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    except Exception as e:  # noqa: BLE001
        node.get_logger().error(f"sensing_status runtime exception: {e}")
    finally:
        node.destroy_node()
        try:
            if rclpy.ok():
