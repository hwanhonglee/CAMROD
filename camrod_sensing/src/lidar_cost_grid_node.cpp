...
public:
  ...
  : Node("lidar_cost_grid")
  {
    ...
    extra_input_topics_ =
      declare_parameter<std::vector<std::string>>("extra_input_topics", std::vector<std::string>{});
    perception_marker_topics_ =
...
private:
  ...
  static void appendUniqueTopic(std::vector<std::string> & topics, const std::string & topic)
  {
    if (topic.empty()) {
      return;
    }
  ...
  void configureInputs()
  {
    ...
    for (std::size_t i = 0; i < input_topics_.size(); ++i) {
      ...
      cloud_inputs_[i].rx_time = rclcpp::Time(0, 0, get_clock()->get_clock_type());
    ...
    for (std::size_t i = 0; i < perception_marker_topics_.size(); ++i) {
      ...
      marker_inputs_[i].rx_time = rclcpp::Time(0, 0, get_clock()->get_clock_type());
    ...
    RCLCPP_INFO(
      get_logger(),
      "lidar_cost_grid: clouds=%zu markers=%zu output=%s",
      cloud_inputs_.size(), marker_inputs_.size(), output_topic_.c_str());
  }

  // Handles the `onCloud` callback.
  void onCloud(const std::size_t idx, const avg_msgs::msg::PointCloud2::ConstSharedPtr msg)
  {
  ...
  void onMarkers(const std::size_t idx, const avg_msgs::msg::MarkerArray::ConstSharedPtr msg)
  {
  ...
  bool getBasePoseInOutput(avg_msgs::msg::PointStamped & base_in_output)
  {
    ...
    // Requesting "now()" can intermittently fail with small future extrapolation
    // during startup/high-load, which causes marker/grid flicker.
  ...
  bool getCloudTransform(
    const avg_msgs::msg::Header & cloud_header,
    ...
    try {
      ...
      try {
        tf_out = tf_buffer_->lookupTransform(
          ...
          rclcpp::Time(0, 0, get_clock()->get_clock_type()), tf2::durationFromSec(0.05));
        return true;
      } catch (const tf2::TransformException & ex_latest) {
        RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 2000,
          "lidar_cost_grid TF lookup failed (%s -> %s): %s",
          cloud_header.frame_id.c_str(), output_frame_id_.c_str(), ex_latest.what());
        return false;
      }
    }
  }
    ...
    avg_msgs::msg::PointStamped & point_out)
  {
    ...
    try {
      point_out = tf_buffer_->transform(
        point, output_frame_id_, tf2::durationFromSec(0.05));
      return true;
    } catch (const tf2::TransformException &) {
      ...
      try {
        ...
        RCLCPP_WARN_THROTTLE(
          ...
          marker.header.frame_id.c_str(), output_frame_id_.c_str(), ex_latest.what());
        return false;
      }
    }
  }
    ...
    const double grid_origin_y)
  {
    ...
    try {
      ...
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 2000,
        "lidar_cost_grid failed to iterate %s: %s", input.topic.c_str(), ex.what());
    }
  }

  // Implements `markMarkerInput` behavior.
  void markMarkerInput(
    ...
    const double grid_origin_y)
  {
    ...
    for (const auto & marker : input.markers->markers) {
      ...
      if (
        marker.type != avg_msgs::msg::Marker::CUBE &&
      ...
      markDiskWithRadius(
        grid, grid_origin_x, grid_origin_y,
        point_out.point.x, point_out.point.y, marker_radius, value);
    }
  }
  ...
  void publishGrid()
  {
    ...
    for (const auto & input : cloud_inputs_) {
      if (input.cloud && (now_time - input.rx_time).seconds() <= max_message_age_s_) {
        has_fresh_input = true;
        markCloudInput(input, grid, base_in_output, grid_origin_x, grid_origin_y);
      }
    }
    for (const auto & input : marker_inputs_) {
      if (input.markers && (now_time - input.rx_time).seconds() <= max_message_age_s_) {
        has_fresh_input = true;
        markMarkerInput(input, grid, base_in_output, grid_origin_x, grid_origin_y);
      }
    }
  ...
  void publishAvgLidar(const avg_msgs::msg::OccupancyGrid & grid)
  {
    ...
    if (latest_primary_cloud_) {
      avg_msg.points_filtered = avg_msgs::conversions::fromRos(*latest_primary_cloud_);
    }
  ...
  std::vector<rclcpp::Subscription<avg_msgs::msg::PointCloud2>::SharedPtr> sub_clouds_;
  std::vector<rclcpp::Subscription<avg_msgs::msg::MarkerArray>::SharedPtr> sub_markers_;
