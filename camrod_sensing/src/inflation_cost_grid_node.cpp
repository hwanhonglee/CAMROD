...
public:
  InflationCostGridNode() : Node("inflation_cost_grid") {
    ...
    enable_robot_frame_window_ =
        declare_parameter<bool>("enable_robot_frame_window", true);
    ...
    rebuild_min_yaw_delta_rad_ =
        declare_parameter<double>("rebuild_min_yaw_delta_rad", 0.03);
    ...
    input_max_ages_s_ = declare_parameter<std::vector<double>>(
        "input_max_ages_s", std::vector<double>{5.0, 0.50, 0.50, 10.0});
    ...
    input_versions_.assign(n, 0);
    last_built_versions_.assign(n, ~std::uint64_t{0});
    input_dynamic_flags_.resize(n, false);
    for (std::size_t i = 0; i < n; ++i) {
      input_dynamic_flags_[i] = isDynamicTopicName(input_topics_[i]);
    }
    ...
    RCLCPP_INFO(get_logger(),
                "inflation_cost_grid ready: %zu inputs -> %s  grid=%dx%d "
                "res=%.2fm window=%s front=%.2fm rear=%.2fm side=%.2fm",
...
private:
  ...
  bool isInsideEgoClear(double wx, double wy, double base_x,
                        double base_y) const {
  ...
  bool isDynamicInputTopic(std::size_t index) const {
    if (index >= input_dynamic_flags_.size())
      return false;
    return input_dynamic_flags_[index];
  }

  static bool isDynamicTopicName(const std::string &topic) {
    return (topic.find("cost_grid/lidar") != std::string::npos ||
            topic.find("cost_grid/radar") != std::string::npos);
  }

  static double yawDelta(double a, double b) {
    return std::abs(std::atan2(std::sin(a - b), std::cos(a - b)));
  }

  bool inputVersionsChanged() const {
    return input_versions_ != last_built_versions_;
  }

  bool poseChangedEnough(double base_x, double base_y, double base_yaw) const {
    if (!cached_grid_valid_) {
      return true;
    }
    const double dx = base_x - last_base_x_;
    const double dy = base_y - last_base_y_;
    return ((dx * dx + dy * dy) >=
                (rebuild_min_pose_delta_m_ * rebuild_min_pose_delta_m_) ||
            yawDelta(base_yaw, last_base_yaw_) >= rebuild_min_yaw_delta_rad_);
  }

  bool cachedRepublishDue(const rclcpp::Time &now_time) const {
    if (cached_republish_period_s_ <= 0.0 ||
        last_publish_time_.nanoseconds() == 0) {
      return true;
    }
    return (now_time - last_publish_time_).seconds() >=
           cached_republish_period_s_;
  }
  ...
  void publishGrid() {
    ...
    try {
      base_in_output = tf_buffer_->lookupTransform(
          output_frame_id_, base_frame_id_, tf2::TimePointZero,
    ...
    if (!inputVersionsChanged() &&
        !poseChangedEnough(base_x, base_y, base_yaw)) {
      if (cached_grid_valid_ && cachedRepublishDue(now_time)) {
    ...
    out.data.assign(static_cast<std::size_t>(width_ * height_),
    ...
    for (int gy = 0; gy < height_; ++gy) {
      for (int gx = 0; gx < width_; ++gx) {
        ...
        if (!insideRobotFrameWindow(wx, wy, base_x, base_y, cos_base_yaw,
                                    sin_base_yaw)) {
          continue;
        }
        ...
        for (std::size_t i = 0; i < input_grids_.size(); ++i) {
          ...
          const double age_limit =
              (i < input_max_ages_s_.size() && input_max_ages_s_[i] > 0.0)
                  ? input_max_ages_s_[i]
                  : max_message_age_s_;
          if ((now_time - input_recv_times_[i]).seconds() > age_limit)
            continue;
    ...
    last_built_versions_ = input_versions_;
    last_base_x_ = base_x;
    last_base_y_ = base_y;
    last_base_yaw_ = base_yaw;
    last_publish_time_ = out.header.stamp;
    pub_grid_->publish(cached_grid_);
  }

  bool insideRobotFrameWindow(double wx, double wy, double base_x,
                              double base_y, double cos_base_yaw,
                              double sin_base_yaw) const {
    if (!enable_robot_frame_window_)
      return true;
    const double dx = wx - base_x;
    const double dy = wy - base_y;
    const double local_x = cos_base_yaw * dx + sin_base_yaw * dy;
    const double local_y = -sin_base_yaw * dx + cos_base_yaw * dy;
    return (local_x <= forward_extent_m_ && local_x >= -rear_extent_m_ &&
  ...
  double rebuild_min_yaw_delta_rad_{0.03};
  double cached_republish_period_s_{0.50};
  std::vector<std::string> input_topics_;
  std::vector<double> input_max_ages_s_;
  std::vector<bool> input_dynamic_flags_;
