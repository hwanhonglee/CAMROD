...
echo "[INFO] Dockerfile amd64: Dockerfile.vio_live_min.amd64"
echo "[INFO] Dockerfile amd64 sha256: $(sha256sum Dockerfile.vio_live_min.amd64 | awk '{print $1}')"
grep -nE "set -euo pipefail|Kimera-VIO CMake configure failed|io\\.lehong\\.profile" Dockerfile.vio_live_min.amd64 || true
echo "[INFO] Dockerfile arm64: Dockerfile.vio_live_min.arm64"
echo "[INFO] Dockerfile arm64 sha256: $(sha256sum Dockerfile.vio_live_min.arm64 | awk '{print $1}')"
grep -nE "set -euo pipefail|Kimera-VIO CMake configure failed|io\\.lehong\\.profile" Dockerfile.vio_live_min.arm64 || true
...
docker buildx build \
  ...
  -f Dockerfile.vio_live_min.amd64 \
  -t "${AMD64_TAG}" \
  "${common_build_args[@]}" \
  --push \
  .
...
docker buildx build \
  ...
  -f Dockerfile.vio_live_min.arm64 \
  -t "${ARM64_TAG}" \
  "${common_build_args[@]}" \
  --push \
  .
