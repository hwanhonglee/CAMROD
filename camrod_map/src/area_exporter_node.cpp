...
struct DropZone
{
  ...
  std::optional<double> service_y;
  std::optional<double> service_z;
  std::optional<double> service_yaw_deg;
};
...
template<typename PrimitiveT>
std::optional<double> getOptionalDoubleAttribute(
  const PrimitiveT & primitive, const std::string & key)
{
  const std::string value = getAttributeCaseInsensitive(primitive, key);
  if (value.empty()) {
    return std::nullopt;
  }
  try {
...
void populateServicePose(const PrimitiveT & primitive, DropZone & zone)
{
  ...
  zone.service_x = getOptionalDoubleAttribute(primitive, "service_x");
  zone.service_y = getOptionalDoubleAttribute(primitive, "service_y");
  zone.service_z = getOptionalDoubleAttribute(primitive, "service_z");
  zone.service_yaw_deg = getOptionalDoubleAttribute(primitive, "service_yaw_deg");
}
...
{
  std::vector<lanelet::BasicPoint3d> flattened;
  flattened.reserve(points.size());
  for (const auto & point : points) {
    flattened.push_back(flattenSemanticPoint(point));
  }
  return flattened;
}
...
{
  const auto yaw_deg = getAttributeCaseInsensitive(primitive, "yaw_deg");
  if (!yaw_deg.empty()) {
    try {
      return std::stod(yaw_deg);
    } catch (const std::exception &) {
      return std::nullopt;
    }
  }
  const auto yaw = getAttributeCaseInsensitive(primitive, "yaw");
  if (!yaw.empty()) {
    try {
      return rad2deg(std::stod(yaw));
    } catch (const std::exception &) {
      return std::nullopt;
    }
  }
...
public:
  ...
  int run()
  {
    ...
    for (const auto & area : map->areaLayer) {
      ...
      dz.yaw_deg = getYawDegOverride(area).value_or(computeYawDegFromPoints(pts, default_yaw_deg_));
      populateServicePose(area, dz);
      if (corners.size() >= 3) {
        dz.corners = flattenSemanticPoints(corners);
      }
      zones.push_back(dz);
    }
    ...
    for (const auto & ls : map->lineStringLayer) {
      ...
      dz.type = zone_type;
      dz.semantic_type = type;
      dz.source = is_area ? "osm_area" : "linestring";
      dz.x = center.x();
      dz.y = center.y();
      dz.z = SEMANTIC_EXPORT_Z_M;
      dz.yaw_deg = getYawDegOverride(ls).value_or(computeYawDegFromPoints(pts, default_yaw_deg_));
      populateServicePose(ls, dz);
      if (corners.size() >= 3) {
        dz.corners = flattenSemanticPoints(corners);
      }
      zones.push_back(dz);
    ...
    for (const auto & pt : map->pointLayer) {
      ...
      dz.y = pt.y();
      dz.z = SEMANTIC_EXPORT_Z_M;
      dz.yaw_deg = getYawDegOverride(pt).value_or(default_yaw_deg_);
      populateServicePose(pt, dz);
      zones.push_back(dz);
    }
    ...
    if (!output_yaml_path_.empty()) {
      ...
      out << "# HH_260721 - Regenerated from the active profile using LocalCartesian runtime coordinates.\n";
      out << "drop_zones:\n";
    ...
    if (!camping_sites_output_yaml_path_.empty()) {
      ...
      out << "# HH_260721 - Preserve map-authored alternate service poses for inaccessible sites.\n";
      out << "camping_sites:\n";
      for (const auto & site : camping_sites) {
        out << "  - id: \"" << site.id << "\"\n";
        ...
        if (!site.service_mode.empty()) {
          // HH_260721 - Emit alternate service targets only for explicitly annotated areas.
          out << "    service_mode: \"" << site.service_mode << "\"\n";
          if (site.service_x.has_value()) {
            out << "    service_x: " << site.service_x.value() << "\n";
          }
          if (site.service_y.has_value()) {
            out << "    service_y: " << site.service_y.value() << "\n";
          }
          if (site.service_z.has_value()) {
            out << "    service_z: " << site.service_z.value() << "\n";
          }
          if (site.service_yaw_deg.has_value()) {
            out << "    service_yaw_deg: " << site.service_yaw_deg.value() << "\n";
          }
        }
...
private:
    ...
    const std::function<std::string(const std::string &)> & classifySemanticType) const
  {
        ...
        if (line.find("<tag") != std::string::npos) {
          ...
          if (key == "type") {
            ...
            try {
              ...
              cur_way.has_yaw_deg = true;
            } catch (const std::exception &) {
              cur_way.has_yaw_deg = false;
            }
          } else if (key == "yaw") {
            try {
              cur_way.yaw_deg = rad2deg(std::stod(val));
              cur_way.has_yaw_deg = true;
            } catch (const std::exception &) {
              cur_way.has_yaw_deg = false;
            }
    ...
    for (const auto & way : ways) {
      ...
      dz.y = center.y();
      dz.z = SEMANTIC_EXPORT_Z_M;
      dz.yaw_deg = way.has_yaw_deg ? way.yaw_deg : computeYawDegFromPoints(pts, default_yaw_deg_);
