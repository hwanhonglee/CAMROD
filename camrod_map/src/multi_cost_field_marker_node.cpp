...
public:
  ...
  : Node("multi_cost_field_marker")
  {
    ...
    const double stale_timeout_default = declare_parameter<double>("stale_timeout_sec_default", 0.0);
    const double min_publish_period_default =
      declare_parameter<double>("min_publish_period_sec_default", 0.0);
    const double republish_period_default =
      declare_parameter<double>("republish_period_sec_default", 0.0);
    ...
    const auto stale_timeout_secs =
      declare_parameter<std::vector<double>>("stale_timeout_secs", std::vector<double>{});
    const auto min_publish_period_secs =
      declare_parameter<std::vector<double>>("min_publish_period_secs", std::vector<double>{});
    const auto republish_period_secs =
      declare_parameter<std::vector<double>>("republish_period_secs", std::vector<double>{});
    ...
    for (size_t i = 0; i < grid_topics.size(); ++i) {
      ...
      stream.stale_timeout_sec = std::max(0.0, pickOrDefault(stale_timeout_secs, i, stale_timeout_default));
      stream.min_publish_period_sec =
        std::max(0.0, pickOrDefault(min_publish_period_secs, i, min_publish_period_default));
      stream.republish_period_sec =
        std::max(0.0, pickOrDefault(republish_period_secs, i, republish_period_default));
