#!/usr/bin/env python3
# HH_260310-00:00 Module-level status_stream publisher for map/cost-grid outputs.
...
from avg_msgs.msg import AvgMapMsgs, ModuleState
...
class MapStatusNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            "map_status: status=%s period=%.2fs"
    ...
    def _state(self, now: Time) -> ModuleState:
        ...
        if missing:
            ...
            out.message = "map streams statey"
        return out
...
def main() -> None:
    ...
    try:
        ...
        node.get_logger().error(f"map_status runtime exception: {e}")
    finally:
        node.destroy_node()
        try:
            if rclpy.ok():
