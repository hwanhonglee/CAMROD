#!/usr/bin/env python3
# HH_260310-00:00 Module-level status_stream publisher for perception outputs.
...
from avg_msgs.msg import AvgPerceptionMsgs, ModuleState
...
class PerceptionStatusNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            "perception_status: status=%s period=%.2fs"
    ...
    def _state(self, now: Time) -> ModuleState:
        ...
        out.message = "perception stream statey"
        return out
...
def main() -> None:
    ...
    try:
        ...
        node.get_logger().error(f"perception_status runtime exception: {e}")
    finally:
        node.destroy_node()
        try:
            if rclpy.ok():
