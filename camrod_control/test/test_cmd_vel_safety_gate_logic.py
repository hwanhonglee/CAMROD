#!/usr/bin/env python3
"""
Core logic unit tests for cmd_vel_safety_gate_node.
...
Test coverage:
  ...
  22. repeated cost-stop detections are log-rate limited
...
[ GNSS Recovery Hold ]
# HH_260720 - The default localization backend is EKF; the gate consumes only its mode contract.
When EKF localization recovers from DR_ONLY(2) to NORMAL(0):
...
# HH_260720 - Stub only the generated contracts and explicit ROS boundaries used by the gate.
# --- ROS 2 stubs (rclpy, tf2_ros, Nav2 boundary messages, etc.) -------------
...
class FakeLogger:
    def __init__(self) -> None:
        # HH_260720 - Retain warnings so log-throttle behavior is testable.
...
# HH_260720 - Stub concrete mission requests used for charger departure.
class PlanningMissionKey:
    def __init__(self, mission_key="", source="test", sec=1, nanosec=0):
...
# HH_260720 - Stub normalized platform CAN/BMS messages for gate safety tests.
class AvgPlatformStatus:
    def __init__(
        self,
        vehicle_state=0,
        control_mode=1,
        ...
        # HH_260720 - Match the canonical platform status fields consumed by the gate.
        ...
        self.battery_state_available = battery_state_available
...
sys.modules.update({
    ...
    "avg_msgs.msg":                 _mod(AvgBool=BoolMsg,
                                           AvgLocalizationMode=AvgLocalizationMode,
                                           AvgOccupancyGrid=OccupancyGrid,
                                           AvgOdometry=Odometry,
                                           AvgPath=Path,
                                           AvgPlatformStatus=AvgPlatformStatus,
                                           AvgPoseStamped=PoseStamped,
                                           AvgTwist=Twist,
                                           ModuleState=ModuleState,
                                           PlanningMissionKey=PlanningMissionKey),
})
for top in ["rcl_interfaces", "geometry_msgs", "avg_msgs"]:
    if top not in sys.modules:
        sys.modules[top] = types.ModuleType(top)

# --- Import target -----------------------------------------------------------

# HH_260720 - Import the flattened control node from the package source directory.
sys.path.insert(0, str(FsPath(__file__).resolve().parents[1] / "src"))
import cmd_vel_safety_gate_node as gmod
...
def make_gate(
...
) -> gmod.CmdVelSafetyGateNode:
    """Create the node instance directly without a ROS 2 runtime."""
    n = gmod.CmdVelSafetyGateNode.__new__(gmod.CmdVelSafetyGateNode)
    ...
    # --- Parameters ---
    # HH_260720 - Unit stubs use the public control command contract.
    n.input_topic = "/control/cmd_vel_raw"
    n.output_topic = "/control/cmd_vel"
    n.engage_topic = "/planning/engage"
    n.mission_engage_topic = "/planning/mission_engage"
    n.mission_request_topic = "/planning/mission_key"
    n.state_topic = "/control/command_enabled"
    # HH_260720 - Geometry-focused tests opt out; a dedicated test verifies operator arming.
    n.require_platform_drive_enable = False
    n._platform_drive_enabled = False
    # HH_260720 - Existing geometry tests opt out; dedicated cases below enable platform safety.
    n.platform_safety_enabled = False
    n.platform_status_timeout_s = 0.5
    n.block_on_platform_status_stale = True
    n.block_on_platform_error_code = True
    n.block_on_charging = True
    n.require_can_control_mode = True
    n.critical_battery_stop_enabled = True
    n.critical_battery_percentage = 0.10
    n.allow_mission_departure_while_charging = True
    n.charging_departure_grace_s = 15.0
    n.charger_departure_mission_prefixes = {"camping_site_"}
    n.mission_request_dedup_s = 1.0
    n.estop_topic_enabled = True
    n.dr_timeout_topic_enabled = True
    n.platform_status_topic = "/platform/status"
    ...
    n._estop = False
    n._estop_sources = {
        n.platform_status_topic: False,
        "/planning/state_machine/estop": False,
    }
    n._dr_timeout = False
    n._platform_status_received_sec = None
    n._platform_vehicle_state = 0
    n._platform_control_mode = 1
    n._platform_error_code = 0
    n._platform_is_charging = False
    n._platform_battery_percentage = None
    n._charging_departure_override_until_sec = 0.0
    n._last_charger_departure_mission_signature = None
    n._last_charger_departure_mission_time_sec = -1.0e9
    ...
    # HH_260720 - Match the runtime repeated-detection log throttle state.
    n._last_cost_detection_log_sec = 0.0
    n._last_cost_detection_log_reason = ""
    ...
    n._drop_zone_maneuver_controller_phase = ""
    n._camping_site_maneuver_controller_phase = ""
...
# HH_260720 - Hardware e-stop is carried by the canonical platform status message.
n._on_platform_status(AvgPlatformStatus(estop=True))
...
n._on_platform_status(AvgPlatformStatus(estop=False))
...
# HH_260720 - Exercise the real command callback path that previously could
# deadlock because the latched state prevented the clear corridor evaluation.
n = make_gate(enable_side_rear=True, enable_cost_latch=True)
n._enabled = True
n.cost_stop_clear_required_s = 2.0
n._last_odom = make_odom(x=0.0, y=0.0, yaw=0.0)
n._last_grid = make_grid(obstacle_x=1.0, obstacle_y=0.0, obstacle_cost=90)
n._on_cmd(forward)
check("callback obstacle command is blocked", abs(n.pub_cmd.last.linear.x) < 0.01)
n._last_grid = make_grid(obstacle_x=1.0, obstacle_y=0.0, obstacle_cost=0)
n._on_cmd(forward)
check("callback first clear command remains blocked", abs(n.pub_cmd.last.linear.x) < 0.01)
n._clock.advance_s(2.1)
n._on_cmd(forward)
check(
    "callback continuous clear releases and forwards",
    not n._cost_stop_latched and abs(n.pub_cmd.last.linear.x - forward.linear.x) < 0.01,
)
...
# ===============================================================================
print("\n=== TEST 18: normalized platform CAN state gates motion ===")
print("  Missing/stale feedback, non-CAN mode, vehicle faults, and error masks block")
n = make_gate()
n.platform_safety_enabled = True
n._enabled = True
check("missing platform status -> blocked", not n._effective_enabled())
n._on_platform_status(AvgPlatformStatus(vehicle_state=0, control_mode=1, error_code=0))
check("normal fresh CAN status -> enabled", n._effective_enabled())
n._on_platform_status(AvgPlatformStatus(vehicle_state=0, control_mode=3, error_code=0))
check("remote-control mode -> blocked", not n._effective_enabled())
n._on_platform_status(AvgPlatformStatus(vehicle_state=2, control_mode=1, error_code=0))
check("vehicle exception -> blocked", not n._effective_enabled())
n._on_platform_status(AvgPlatformStatus(vehicle_state=0, control_mode=1, error_code=0x0100))
check("motor-driver error mask -> blocked", not n._effective_enabled())
n._on_platform_status(AvgPlatformStatus(vehicle_state=0, control_mode=1, error_code=0))
n._clock.advance_s(0.6)
check("stale platform status -> blocked", not n._effective_enabled())


# ===============================================================================
print("\n=== TEST 19: charger departure requires a campsite mission request ===")
print("  A concrete campsite request works even when mission engage is already latched")
n = make_gate()
n.platform_safety_enabled = True
# HH_260720 - Charging state arrives with the canonical CAN/BMS platform status.
n._on_platform_status(
    AvgPlatformStatus(vehicle_state=0, control_mode=1, error_code=0, is_charging=True)
)
n._manual_enabled = True
n._enabled = True
check("charging with manual engage -> blocked", not n._effective_enabled())
n._manual_enabled = False
n._mission_enabled = True
n._enabled = True
n._on_mission_engage(BoolMsg(True))
check("repeated mission engage while charging -> still blocked", not n._effective_enabled())
n._on_mission_request(PlanningMissionKey("drop_zone", sec=2))
check("non-campsite mission while charging -> blocked", not n._effective_enabled())
n._on_mission_request(PlanningMissionKey("camping_site_3", sec=3))
check("new campsite request while charging -> departure enabled", n._effective_enabled())
check(
    "gate status reports DEPARTING_CHARGER",
    "state=DEPARTING_CHARGER" in n.pub_status.last.message,
)
departure_deadline = n._charging_departure_override_until_sec
n._clock.advance_s(0.5)
n._on_mission_request(PlanningMissionKey("camping_site_3", sec=3))
check(
    "duplicate campsite request does not extend departure window",
    n._charging_departure_override_until_sec == departure_deadline,
)
n._clock.advance_s(14.6)
check("charging departure grace expiry -> blocked", not n._effective_enabled())
n._on_platform_status(
    AvgPlatformStatus(vehicle_state=0, control_mode=1, error_code=0, is_charging=False)
)
check("charging current cleared -> mission driving enabled", n._effective_enabled())


# ===============================================================================
print("\n=== TEST 20: critical BMS SOC blocks motion ===")
n = make_gate()
n.platform_safety_enabled = True
n._enabled = True
# HH_260720 - BMS SOC is normalized in the platform bridge and carried in AvgPlatformStatus.
n._on_platform_status(
    AvgPlatformStatus(
        vehicle_state=0,
        control_mode=1,
        error_code=0,
        battery_percentage=0.05,
        battery_state_available=True,
    )
)
check("canonical 5 percent SOC blocks", not n._effective_enabled())
check("canonical SOC remains 0.05", math.isclose(n._platform_battery_percentage, 0.05))


# ===============================================================================
print("\n=== TEST 21: platform operator arm is enforced by the control gate ===")
n = make_gate()
n.require_platform_drive_enable = True
n._enabled = True
check("unarmed platform -> blocked", not n._effective_enabled())
n._on_platform_drive_enable(BoolMsg(True))
check("armed platform -> enabled", n._effective_enabled())
n._on_platform_drive_enable(BoolMsg(False))
check("disarming platform -> blocked", not n._effective_enabled())
check("disarming platform publishes zero", abs(n.pub_cmd.last.linear.x) < 0.01)


# ===============================================================================
print("\n=== TEST 22: repeated cost-stop warnings are throttled ===")
print("  The same obstacle remains blocking while duplicate warnings are rate limited")
n = make_gate()
n._enabled = True
n._last_odom = make_odom(x=0.0, y=0.0, yaw=0.0)
n._last_grid = make_grid(obstacle_x=1.0, obstacle_y=0.0, obstacle_cost=90)
# HH_260720 - Three identical high-rate callbacks should produce one warning.
for _ in range(3):
    n._should_stop_for_cost(forward)
check("same obstacle still blocks every callback", n._should_stop_for_cost(forward))
check("same obstacle warning emitted once within interval", len(n._logger.warn_messages) == 1)
n._clock.advance_s(1.1)
n._should_stop_for_cost(forward)
check("same obstacle warning repeats after interval", len(n._logger.warn_messages) == 2)
...
print(f"{'='*62}\n")
# HH_260720 - Expose the custom 78-check suite as one collected pytest result.
def test_all_cmd_vel_safety_gate_checks_pass() -> None:
    assert passed == total, f"cmd_vel safety gate checks failed: {passed}/{total}"
