...
inline RouteRecoveryCandidateKind uniqueClearKind(
...
inline RouteRecoveryCandidateKind projectedReverseYawKind(
  const avg_msgs::msg::AvgTwist & trigger,
  const RouteRecoveryCandidateDecisions & decisions)
{
  const auto unique = uniqueClearKind(
    decisions.reverse_yaw_left, decisions.reverse_yaw_right,
  ...
  if (decisions.reverse_yaw_left.blocked || decisions.reverse_yaw_right.blocked) {
    return RouteRecoveryCandidateKind::kNone;
  }
...
inline const MotionCostStopDecision * routeRecoveryDecision(
  const RouteRecoveryCandidateDecisions & decisions,
  const RouteRecoveryCandidateKind kind)
{
  switch (kind) {
...
inline RouteRecoveryCandidate makeRouteRecoveryCandidate(
  const avg_msgs::msg::AvgTwist & trigger,
  const RouteRecoveryCandidateKind kind,
  const double speed_mps,
  ...
  const std::string & reason)
{
  return {
    ...
    routeRecoveryDirection(
      trigger, kind, speed_mps, minimum_translation_mps, yaw_rate_radps),
    reason};
}
  ...
  const double minimum_translation_mps = 0.02)
{
  ...
  const auto lateral = uniqueClearKind(
    decisions.crab_left, decisions.crab_right,
  ...
  if (lateral != RouteRecoveryCandidateKind::kNone && lateral != active_kind) {
    return makeRouteRecoveryCandidate(
      ...
      "unique_lateral_clear" : "stage_switch_to_unique_lateral");
  }
  ...
  if (active_kind != RouteRecoveryCandidateKind::kNone) {
    ...
    if (active_decision != nullptr && !active_decision->blocked) {
      ...
      return makeRouteRecoveryCandidate(
        trigger, active_kind, speed_mps, yaw_rate_radps,
        minimum_translation_mps, "active_stage_still_clear");
    }
  }
  ...
  if (!decisions.reverse.blocked) {
    return makeRouteRecoveryCandidate(
      ...
      active_kind == RouteRecoveryCandidateKind::kNone ?
      "lateral_blocked_reverse_clear" : "active_stage_blocked_reverse_reposition");
  }
  if (reverse_yaw != RouteRecoveryCandidateKind::kNone) {
    return makeRouteRecoveryCandidate(
      trigger, reverse_yaw, speed_mps, yaw_rate_radps,
      minimum_translation_mps, "unique_reverse_yaw_clear");
  }
