// HH_260721 - Implement campsite crab, zero-turn, recall, and reverse-entry maneuvers in C++.
...
public:
  ...
  : Node("camping_site_maneuver_controller")
  {
    ...
    site_mission_key_prefix_ = declare_parameter<std::string>(
      "site_mission_key_prefix",
      "camping_site_");
    use_goal_pair_for_lateral_offset_ = declare_parameter<bool>(
      "use_goal_pair_for_lateral_offset",
      true);
    require_goal_pair_for_auto_start_ = declare_parameter<bool>(
      "require_goal_pair_for_auto_start",
      true);
    site_entry_mode_ = declare_parameter<std::string>("site_entry_mode", "crab");
    ...
    entry_position_tolerance_m_ = std::abs(
      declare_parameter<double>("entry_position_tolerance_m", 0.15));
    ...
    crab_timeout_margin_s_ = std::abs(declare_parameter<double>("crab_timeout_margin_s", 3.0));
    ...
    crab_timeout_speed_scale_ = std::max(
      0.05, std::abs(declare_parameter<double>("crab_timeout_speed_scale", 1.0)));
    maximum_forward_residual_m_ = std::abs(
      declare_parameter<double>("max_forward_residual_m", 0.8));
    rotate_proportional_gain_ = declare_parameter<double>("rotate_kp", 1.2);
    maximum_angular_speed_radps_ = std::abs(
      declare_parameter<double>("max_angular_speed_radps", 0.35));
    rotate_yaw_tolerance_deg_ = std::abs(
      declare_parameter<double>("rotate_yaw_tolerance_deg", 4.0));
    ...
    {
      RCLCPP_WARN(
        get_logger(), "unknown site_rotate_direction_policy='%s'; using shortest",
        site_rotate_direction_policy_.c_str());
      site_rotate_direction_policy_ = "shortest";
    }
    const std::vector<int64_t> right_indices = declare_parameter<std::vector<int64_t>>(
      "right_lanelet_site_indices", {1, 3, 5, 7, 9, 11});
    right_lanelet_site_indices_.insert(right_indices.begin(), right_indices.end());
    const std::vector<int64_t> left_indices = declare_parameter<std::vector<int64_t>>(
      "left_lanelet_site_indices", {2, 4, 6, 8, 10, 12, 13});
    left_lanelet_site_indices_.insert(left_indices.begin(), left_indices.end());

    reverse_entry_speed_mps_ = std::abs(declare_parameter<double>("reverse_entry_speed_mps", 0.16));
    reverse_entry_yaw_gain_ = declare_parameter<double>("reverse_entry_yaw_kp", 1.0);
    reverse_entry_lateral_gain_ = declare_parameter<double>("reverse_entry_lateral_kp", -0.25);
    reverse_entry_maximum_angular_speed_radps_ = std::abs(
      declare_parameter<double>("reverse_entry_max_angular_speed_radps", 0.25));
    reverse_entry_use_site_goal_yaw_ = declare_parameter<bool>(
    ...
    {
      RCLCPP_WARN(
        get_logger(), "unknown reverse_entry_site_yaw_mode='%s'; using reverse_axis",
    ...
    reverse_entry_debug_period_s_ = declare_parameter<double>("reverse_entry_debug_period_s", 1.0);
    ...
    cancel_nav2_on_site_phase_ = declare_parameter<bool>("cancel_nav2_on_site_phase", true);
    nav2_cancel_period_s_ = declare_parameter<double>("nav2_cancel_period_s", 0.5);
    ...
    goal_pair_maximum_age_s_ = declare_parameter<double>("goal_pair_max_age_s", 0.0);
    control_rate_hz_ = std::max(0.1, declare_parameter<double>("control_rate_hz", 10.0));
    idle_tick_rate_hz_ = std::max(0.1, declare_parameter<double>("idle_tick_rate_hz", 1.0));
    status_publish_rate_hz_ = std::max(
      0.1, declare_parameter<double>("status_publish_rate_hz", 1.0));
    ...
    site_goal_subscription_ = create_subscription<geometry_msgs::msg::PoseStamped>(
      site_goal_topic_, 10,
      [this](const geometry_msgs::msg::PoseStamped::SharedPtr message) {
        onSiteGoal(camrod_control::poseFromRos(*message));
      });
    route_goal_subscription_ = create_subscription<avg_msgs::msg::AvgPoseStamped>(
    ...
    adopt_destination_subscription_ = create_subscription<avg_msgs::msg::UiDestinationCommand>(
      adopt_destination_topic_, 10,
      [this](const avg_msgs::msg::UiDestinationCommand::SharedPtr message) {
        if (message->run) {
          const std::string key = missionKeyFromDestination(*message);
          if (!key.empty()) {
            adoptWaitReturnState(
              key, message->source.empty() ? "adopt_destination" : message->source);
          }
        }
      });
    ...
    RCLCPP_INFO(
      get_logger(),
      "camping_site_maneuver_controller ready: cmd=%s pose=%s lanelet_pose=%s auto_start=%s entry_mode=%s adopt_topic=%s cancel_nav2=%s",
      command_topic_.c_str(), pose_topic_.c_str(), lanelet_pose_topic_.c_str(),
      enable_auto_start_from_planning_state_ ? "true" : "false", site_entry_mode_.c_str(),
      adopt_destination_topic_.c_str(), cancel_nav2_on_site_phase_ ? "true" : "false");
  }
...
private:
    ...
    const double yaw_deg) const
  {
    ...
    pose.header.frame_id = frame_id.empty() ?
      (site_goal_frame_id_.empty() ? "map" : site_goal_frame_id_) : frame_id;
  ...
  avg_msgs::msg::AvgPoseStamped stampPoseNow(avg_msgs::msg::AvgPoseStamped pose) const
  {
    pose.header.stamp = now();
    if (pose.header.frame_id.empty()) {
      pose.header.frame_id = site_goal_frame_id_.empty() ? "map" : site_goal_frame_id_;
    }
    return pose;
  }
  ...
  void loadCampingSites()
  {
    ...
    try {
      ...
      RCLCPP_INFO(
        get_logger(), "loaded %zu campsite goal entries from %s",
  ...
  static double poseDistance(
    const avg_msgs::msg::AvgPoseStamped & first,
  ...
  void onSiteGoal(const avg_msgs::msg::AvgPoseStamped & message)
  {
    if (isSiteInternalPhase()) {
      ...
      RCLCPP_WARN(
        get_logger(),
        "camping_site_maneuver_controller ignored new site_goal while internal: phase=%s x=%.2f y=%.2f",
        phaseName(phase_).c_str(), message.pose.position.x, message.pose.position.y);
      return;
    }
    const bool had_goal = site_goal_.has_value();
    const bool goal_changed = had_goal && poseDistance(*site_goal_, message) > 0.2;
    site_goal_ = message;
    site_goal_time_ = now();
    site_goal_key_.clear();
    if (!had_goal || goal_changed || phase_ == CampingSiteManeuverPhase::kIdle ||
      phase_ == CampingSiteManeuverPhase::kDone || phase_ == CampingSiteManeuverPhase::kError)
    {
      last_auto_key_.clear();
    }
    if (isActivePhase() && goal_changed) {
      ...
      setPhase(CampingSiteManeuverPhase::kIdle, "new site_goal received; stale maneuver cancelled");
    }
    RCLCPP_INFO(
      get_logger(), "camping_site_maneuver_controller site_goal updated: topic=%s x=%.2f y=%.2f",
      site_goal_topic_.c_str(), message.pose.position.x, message.pose.position.y);
  }

  std::string missionKeyFromDestination(const avg_msgs::msg::UiDestinationCommand & message) const
  {
    ...
    {
      return site_mission_key_prefix_ + std::to_string(std::stoi(site.substr(1)));
    }
    return site;
  }
    ...
    const avg_msgs::msg::AvgPoseStamped & site_goal) const
  {
    ...
    return distance >= minimum_lateral_offset_m_ && distance <= maximum_lateral_offset_m_;
  }
  ...
  bool adoptWaitReturnState(const std::string & key, const std::string & source)
  {
    ...
    {
      RCLCPP_WARN(
        get_logger(), "camping_site_maneuver_controller adopt ignored during phase=%s",
        phaseName(phase_).c_str());
      return false;
    }
    if (!poseIsFresh()) {
      RCLCPP_WARN(
        get_logger(), "camping_site_maneuver_controller adopt ignored: fresh pose unavailable");
      return false;
    }
    ...
    if (!adopted_site_goal.has_value()) {
      RCLCPP_WARN(
        get_logger(), "camping_site_maneuver_controller adopt ignored: site goal unavailable for %s",
        key.c_str());
      return false;
    }
    *adopted_site_goal = stampPoseNow(*adopted_site_goal);
    const double distance_to_site = poseDistance(*last_pose_, *adopted_site_goal);
    if (distance_to_site > adopt_site_arrival_distance_m_) {
      RCLCPP_WARN(
        get_logger(), "camping_site_maneuver_controller adopt ignored: key=%s distance=%.2fm limit=%.2fm",
        key.c_str(), distance_to_site, adopt_site_arrival_distance_m_);
      return false;
    }
    ...
    const auto relative = camrod_control::relativeXy(adopted_route_goal, *adopted_site_goal);
    ...
    if (offset < minimum_lateral_offset_m_) {
      ...
      if (route_source != "current_pose_fallback") {
        offset = std::min(
          poseDistance(
            adopted_route_goal,
            *adopted_site_goal), maximum_lateral_offset_m_);
    ...
    const double effective_speed = std::max(0.01, crab_speed_mps_ * crab_timeout_speed_scale_);
  ...
  void ensureGoalPairForAutoStart(const std::string & key)
  {
    ...
    if (!route_goal_.has_value() && poseIsFresh()) {
      ...
      route_goal_time_ = now();
      RCLCPP_INFO(get_logger(), "restored route_goal from current pose at GOAL_REACHED");
    }
  }
  ...
  std::pair<double, std::string> rotationDirectionForSource(const std::string & source) const
  {
    if (site_rotate_direction_policy_ != "site_index_lanelet_side") {
      return {0.0, "shortest"};
    }
  ...
  {
    if (message.scenario_id == avg_msgs::msg::PlanningScenario::RETURN_TO_DROP_ZONE ||
    ...
    if (message.scenario_id != avg_msgs::msg::PlanningScenario::DELIVERY_TO_SITE &&
      message.scenario_id != avg_msgs::msg::PlanningScenario::RECALL_TO_SITE)
    {
      return;
    }
  ...
  std::pair<bool, std::string> applyOperation(const uint8_t operation, const std::string & source)
  {
    ...
    return {false, "unsupported camping-site operation=" + std::to_string(operation)};
  }
  ...
  std::tuple<double, double, std::string, double> resolveLateralMotion() const
  {
    ...
    {
      const auto relative = camrod_control::relativeXy(*route_goal_, *site_goal_);
    ...
    return {
      camrod_control::clamp(offset, minimum_lateral_offset_m_, maximum_lateral_offset_m_),
  ...
  std::pair<bool, std::string> startSequence(const std::string & source)
  {
    ...
    // HH_260807 - Nav2 may report GOAL_REACHED before the body reaches the exact
    // snapped centerline pose. Preserve the actual pose for entry progress, but
    // return automatic services to the explicit route goal so the next Nav2
    // command never inherits an arrival pose already touching the road margin.
    captureReturnAnchor(source.rfind("planning_state:", 0) == 0 && source_name == "goal_pair");
    ...
    entry_reference_yaw_ = start_yaw_;
    crab_duration_s_ = lateral_offset /
      std::max(0.01, crab_speed_mps_ * crab_timeout_speed_scale_);
    return_requested_ = false;
    return_published_ = false;
    return_acknowledged_ = false;
    last_return_request_publish_time_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
    if (source_name == "goal_pair" && std::abs(forward_residual) > maximum_forward_residual_m_) {
      RCLCPP_WARN(
        get_logger(), "goal pair forward residual %.2fm exceeds crab-only limit %.2fm",
    ...
    setPhase(
      ...
      fixed(return_anchor_offset_m_) +
      "m anchor_xy=(" + fixed(return_anchor_x_) + "," + fixed(return_anchor_y_) + ")" +
  ...
  std::pair<double, std::string> selectReverseEntryAxis(
    const double direct_axis_yaw,
    const double dx,
    const double dy) const
  {
    ...
    if (reverse_entry_auto_select_yaw_equivalent_) {
      const double forward_projection = std::cos(axis_yaw) * dx + std::sin(axis_yaw) * dy;
      const double flipped_axis = camrod_control::normalizeAngle(axis_yaw + M_PI);
      const double flipped_projection = std::cos(flipped_axis) * dx + std::sin(flipped_axis) * dy;
  ...
  std::pair<bool, std::string> startReverseEntrySequence(
    ...
    const double forward_residual)
  {
    ...
    if (distance > maximum_lateral_offset_m_) {
      setError(
        "reverse site target distance " + fixed(distance) +
        "m exceeds max " + fixed(maximum_lateral_offset_m_) + "m");
      return {false, "reverse site target exceeds max distance"};
    }
    start_yaw_ = camrod_control::yawFromPose(*last_pose_);
    const auto selected_axis = selectReverseEntryAxis(std::atan2(dy, dx), dx, dy);
    entry_reverse_axis_yaw_ = selected_axis.first;
    entry_target_yaw_ = camrod_control::normalizeAngle(entry_reverse_axis_yaw_ + M_PI);
    ...
    crab_source_ = source_name;
    goal_pair_forward_m_ = forward_residual;
    crab_duration_s_ = distance /
      std::max(0.01, reverse_entry_speed_mps_ * crab_timeout_speed_scale_);
    return_requested_ = false;
    return_published_ = false;
    return_acknowledged_ = false;
    last_return_request_publish_time_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
    publishReversePath(
      entry_start_x_, entry_start_y_, entry_target_x_, entry_target_y_, entry_target_yaw_);
    setPhase(
      CampingSiteManeuverPhase::kAlignEntryYaw,
      "start=" + source + " reverse_distance=" + fixed(distance) +
      "m target=(" + fixed(entry_target_x_) + "," + fixed(entry_target_y_) +
      ") source=" + source_name + " forward=" + fixed(forward_residual) +
      "m axis=" + selected_axis.second + " axis_yaw=" +
      fixed(entry_reverse_axis_yaw_ * 180.0 / M_PI, 1) + "deg body_yaw=" +
      fixed(entry_target_yaw_ * 180.0 / M_PI, 1) + "deg");
  ...
  std::pair<bool, std::string> requestReturn(const std::string & source)
  {
    ...
    if (phase_ == CampingSiteManeuverPhase::kIdle || phase_ == CampingSiteManeuverPhase::kDone) {
      return {false, "site maneuver is not waiting for return: " + phaseName(phase_)};
    }
    return_requested_ = true;
  ...
  bool routeGoalReachedForAutoStart() const
  {
    return route_goal_.has_value() && poseIsFresh() &&
           poseDistance(*last_pose_, *route_goal_) <= route_goal_reached_distance_m_;
  }

  bool poseIsFresh() const
  {
    return last_pose_.has_value() && (now() - last_pose_time_).seconds() <= pose_timeout_s_;
  }
  ...
  double relativeLateralFromEntry() const
  {
    ...
    return -std::sin(entry_reference_yaw_) * dx + std::cos(entry_reference_yaw_) * dy;
  }

  bool entryReached() const
  {
    return poseIsFresh() && crab_direction_ * relativeLateralFromEntry() >=
  ...
  std::pair<double, double> axisProgressLateral(
  ...
  double distanceTo(const double x, const double y) const
  {
    return last_pose_.has_value() ?
           std::hypot(last_pose_->pose.position.x - x, last_pose_->pose.position.y - y) :
  ...
  bool reverseInReached() const
  {
    ...
    const double progress = axisProgressLateral(
      entry_start_x_, entry_start_y_, entry_reverse_axis_yaw_).first;
    const double target_line_lateral = axisProgressLateral(
      entry_line_origin_x_, entry_line_origin_y_, entry_reverse_axis_yaw_).second;
    return distanceTo(entry_target_x_, entry_target_y_) <= entry_position_tolerance_m_ ||
           (progress >= std::max(0.0, entry_target_progress_m_ - entry_position_tolerance_m_) &&
           std::abs(target_line_lateral) <= reverse_entry_lateral_tolerance_m_);
  }
  ...
  bool reverseOutReached() const
  {
    ...
    if (distanceTo(return_anchor_x_, return_anchor_y_) <= return_position_tolerance_m_) {
      return true;
    }
    const double return_axis = camrod_control::normalizeAngle(entry_reverse_axis_yaw_ + M_PI);
    const double target_dx = return_anchor_x_ - return_start_x_;
    const double target_dy = return_anchor_y_ - return_start_y_;
    const double target_progress = std::cos(return_axis) * target_dx +
      std::sin(return_axis) * target_dy;
    const double current_progress = axisProgressLateral(
      return_start_x_, return_start_y_, return_axis).first;
    const double lateral_to_snap_line = axisProgressLateral(
      return_anchor_x_, return_anchor_y_, return_axis).second;
  ...
  void setPhase(const CampingSiteManeuverPhase phase, const std::string & detail)
  {
    phase_ = phase;
    phase_start_time_ = now();
    if (phase_ == CampingSiteManeuverPhase::kReverseOut && last_pose_.has_value()) {
    ...
    RCLCPP_INFO(
      get_logger(), "camping_site_maneuver_controller %s: %s",
  ...
  void publishCrabReturnToAnchor()
  {
    ...
    const double remaining_distance = distanceTo(return_anchor_x_, return_anchor_y_);
    ...
    const auto velocity = camrod_control::bodyTranslationTowardTarget(
      ...
      return_anchor_x_, return_anchor_y_, return_speed);
    avg_msgs::msg::AvgTwist command;
    ...
    const double robot_yaw) const
  {
    ...
    path.header.frame_id = last_pose_.has_value() && !last_pose_->header.frame_id.empty() ?
      last_pose_->header.frame_id : (site_goal_frame_id_.empty() ? "map" : site_goal_frame_id_);
  ...
  void requestNav2CancelForSitePhase(const bool force)
  {
    ...
    for (const auto & client : nav2_cancel_clients_) {
      ...
      client->async_send_request(std::make_shared<action_msgs::srv::CancelGoal::Request>());
      sent = true;
    }
    if (sent) {
        ...
        (current_time - last_nav2_cancel_log_time_).seconds() >= 2.0)
      {
        ...
        RCLCPP_INFO(
          get_logger(), "requested Nav2 cancel during %s", phaseName(phase_).c_str());
      }
    }
  }

  void publishReverseAlongAxis(
    ...
    const double origin_y)
  {
    ...
    const double reverse_axis = camrod_control::normalizeAngle(target_yaw + M_PI);
    const double lateral_error = axisProgressLateral(origin_x, origin_y, reverse_axis).second;
    avg_msgs::msg::AvgTwist command;
    command.linear.x = -reverse_entry_speed_mps_;
    command.angular.z = camrod_control::clamp(
      reverse_entry_yaw_gain_ * heading_error + reverse_entry_lateral_gain_ * lateral_error,
    ...
    if (reverse_entry_debug_period_s_ > 0.0 &&
      ...
      (current_time - last_reverse_entry_debug_time_).seconds() >= reverse_entry_debug_period_s_))
    {
      last_reverse_entry_debug_time_ = current_time;
      RCLCPP_INFO(
        get_logger(),
        "reverse control: phase=%s target_yaw=%.1fdeg reverse_axis=%.1fdeg heading_err=%.1fdeg lateral_err=%.2fm angular_z=%.2f",
  ...
  bool publishRotate()
  {
    ...
    if (phase_ == CampingSiteManeuverPhase::kRotate180 && rotate_direction_sign_ != 0.0) {
      ...
      if (std::abs(remaining * 180.0 / M_PI) <= rotate_yaw_tolerance_deg_) {
        publishZero();
        return true;
      }
      avg_msgs::msg::AvgTwist command;
      if (rotate_direction_sign_ * remaining > 0.0) {
        command.angular.z = rotate_direction_sign_ * std::min(
          maximum_angular_speed_radps_,
          std::max(0.05, std::abs(rotate_proportional_gain_ * remaining)));
      } else {
        command.angular.z = camrod_control::clamp(
          rotate_proportional_gain_ * camrod_control::normalizeAngle(target_yaw_ - yaw),
    ...
    if (std::abs(error * 180.0 / M_PI) <= rotate_yaw_tolerance_deg_) {
      publishZero();
      return true;
    }
    avg_msgs::msg::AvgTwist command;
    command.angular.z = camrod_control::clamp(
      rotate_proportional_gain_ * error,
      -maximum_angular_speed_radps_, maximum_angular_speed_radps_);
    command_publisher_->publish(command);
    return false;
  }
  ...
  void publishReturnRequest(const std::string & reason)
  {
    ...
    if (return_published_ && return_request_retry_period_s_ > 0.0 &&
      (current_time - last_return_request_publish_time_).seconds() < return_request_retry_period_s_)
    {
      return;
    }
    ...
    RCLCPP_INFO(get_logger(), "return_to_drop_zone requested: %s", reason.c_str());
  }
  ...
  void onTimer()
  {
    ...
    if (phase_ == CampingSiteManeuverPhase::kAlignEntryYaw) {
      if (publishRotate()) {
        setPhase(
          CampingSiteManeuverPhase::kReverseIn,
      ...
      if (reverseInReached()) {
        ...
        setPhase(CampingSiteManeuverPhase::kRotate180, "reverse entry pose reached");
      } else if (elapsed > crab_duration_s_ + crab_timeout_margin_s_) {
        setError("reverse entry timeout before reaching site target");
      } else {
        publishReverseAlongAxis(entry_target_yaw_, entry_line_origin_x_, entry_line_origin_y_);
      }
      ...
      if (publishRotate()) {
        setPhase(CampingSiteManeuverPhase::kUnloadWait, "robot yaw rotated 180deg");
      }
    } else if (phase_ == CampingSiteManeuverPhase::kUnloadWait) {
      publishZero();
      if (elapsed >= unload_wait_s_) {
        if (auto_return_after_unload_wait_ || return_requested_) {
          ...
          setPhase(CampingSiteManeuverPhase::kWaitReturn, "waiting external return command");
        }
      }
    } else if (phase_ == CampingSiteManeuverPhase::kWaitReturn) {
      publishZero();
      ...
      } else if (elapsed > crab_duration_s_ + reverse_return_timeout_margin_s_) {
      ...
      if (returnReached()) {
        ...
        if (active_service_mode_ == CampsiteServiceMode::kRoadsideStop) {
          ...
          setPhase(
            ...
            " error=" + fixed(distanceTo(return_anchor_x_, return_anchor_y_)) + "m");
          publishReturnRequest("done");
        }
      } else if (elapsed > crab_duration_s_ + crab_timeout_margin_s_) {
  ...
  void publishStatus(const bool force)
  {
    ...
    if (!force && last_status_publish_time_.nanoseconds() > 0 &&
      (current_time - last_status_publish_time_).seconds() < 1.0 / status_publish_rate_hz_)
    {
      return;
    }
    ...
    diagnostics_publisher_->publish(
      camrod_control::makeDiagnostics(
        *this, "control/camping_site_maneuver_controller", "control", diagnostic_level, message,
    {
      {"phase", phaseName(phase_)},
  ...
  rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr site_goal_subscription_;
  rclcpp::Subscription<avg_msgs::msg::AvgPoseStamped>::SharedPtr route_goal_subscription_;
