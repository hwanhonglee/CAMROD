...
# HH_260720 - Move campsite motion ownership from parking to control without changing the maneuver sequence.
...
class Phase(str, Enum):
    ...
    ALIGN_RETURN_YAW = "ALIGN_RETURN_YAW"
...
def quat_from_yaw(yaw: float):
    # HH_260720 - Construct the generated quaternion used by internal maneuver poses.
    q = AvgPoseStamped().pose.orientation
    q.z = math.sin(yaw * 0.5)
    q.w = math.cos(yaw * 0.5)
    return q


class CampingSiteManeuverControllerNode(Node):
    def __init__(self) -> None:
        # HH_260720 - Use a node name that states the controlled location and responsibility.
        super().__init__("camping_site_maneuver_controller")

        # HH_260720 - Publish maneuver candidates into the control-owned command path.
        self.cmd_vel_topic = str(self.declare_parameter("cmd_vel_topic", "/control/cmd_vel_raw").value)
        self.pose_topic = str(self.declare_parameter("pose_topic", "/localization/pose").value)
        # HH_260720 - Keep only the raw operator goal as a standard ROS boundary;
        # snapped route and lanelet poses use generated CAMROD messages internally.
        self.site_goal_topic = str(self.declare_parameter("site_goal_topic", "/goal_pose").value)
        self.route_goal_topic = str(
            self.declare_parameter("route_goal_topic", "/planning/goal_pose_snapped").value
        )
        self.lanelet_pose_topic = str(
            self.declare_parameter("lanelet_pose_topic", "/planning/lanelet_pose").value
        )
        ...
        self.adopt_destination_topic = str(
            self.declare_parameter("adopt_destination_topic", "/control/camping_site_maneuver_controller/adopt").value
        )
        self.status_topic = str(
            self.declare_parameter("status_topic", "/control/camping_site_maneuver_controller/status").value
        )
        self.diagnostics_topic = str(self.declare_parameter("diagnostics_topic", "/system/diagnostics").value)
        # HH_260720 - Control maneuver phases continue updating the shared UI service state.
        ...
        self.reverse_path_topic = str(
            self.declare_parameter("reverse_path_topic", "/control/camping_site_maneuver_controller/path_ros").value
        )
        ...
        # HH_260720 - The control safety gate can scale `/control/cmd_vel_raw`
        ...
        # HH_260701 - Restore lanelet-snap yaw inside the campsite before
        # crab-out so the Nav2 return route does not begin with another 180deg turn.
        self.align_return_yaw_before_crab_out = bool(
            self.declare_parameter("align_return_yaw_before_crab_out", True).value
        )
        ...
        self.cancel_nav2_on_site_phase = bool(
            self.declare_parameter("cancel_nav2_on_site_phase", True).value
        )
        self.nav2_cancel_period_s = float(
            self.declare_parameter("nav2_cancel_period_s", 0.5).value
        )
        nav2_cancel_topics_raw = self.declare_parameter(
        ...
        if isinstance(nav2_cancel_topics_raw, str):
            self.nav2_cancel_action_topics = [
                item.strip() for item in nav2_cancel_topics_raw.split(",") if item.strip()
            ]
        else:
            self.nav2_cancel_action_topics = [
                str(item).strip() for item in nav2_cancel_topics_raw if str(item).strip()
            ]
        ...
        self.adopt_site_arrival_distance_m = abs(
            float(self.declare_parameter("adopt_site_arrival_distance_m", 3.0).value)
        )
        ...
        # HH_260720 - Publish generated commands and semantic recall requests internally.
        self.cmd_pub = self.create_publisher(AvgTwist, self.cmd_vel_topic, 10)
        ...
        self.return_request_pub = self.create_publisher(
            PlanningRecallRequest, self.return_to_drop_zone_topic, 10
        )
        ...
        self.nav2_cancel_clients = [
            self.create_client(CancelGoal, topic)
            for topic in self.nav2_cancel_action_topics
        ]

        self.create_subscription(AvgPoseStamped, self.pose_topic, self._on_pose, 10)
        # HH_260720 - Convert the UI goal once; all control-owned pose state stays generated.
        self.create_subscription(RosPoseStamped, self.site_goal_topic, self._on_site_goal_ros, 10)
        self.create_subscription(AvgPoseStamped, self.route_goal_topic, self._on_route_goal, 10)
        self.create_subscription(AvgPoseStamped, self.lanelet_pose_topic, self._on_lanelet_pose, 10)
        ...
        self.create_subscription(
            UiDestinationCommand,
            self.adopt_destination_topic,
            self._on_adopt_destination,
            10,
        )
        # HH_260720 - One typed service replaces duplicate start/return/cancel Trigger services.
        self.create_service(
        ...
        self.last_pose: AvgPoseStamped | None = None
        self.last_pose_time_s = 0.0
        self.site_goal: AvgPoseStamped | None = None
        self.route_goal: AvgPoseStamped | None = None
        self.lanelet_pose: AvgPoseStamped | None = None
        self.site_goal_time_s = 0.0
        self.route_goal_time_s = 0.0
        self.lanelet_pose_time_s = 0.0
        self.site_goal_key = ""
        ...
        self.return_crab_direction = 1.0
        ...
        self.last_nav2_cancel_request_s = -1.0e9
        self.last_nav2_cancel_log_s = -1.0e9
        self.last_reverse_entry_debug_s = 0.0
        self.last_status_publish_s = 0.0
        self.camping_site_goals: dict[str, AvgPoseStamped] = {}
        self._load_camping_sites()

        self.timer = self.create_timer(self._timer_period_s(), self._tick)
        self.get_logger().info(
            "camping_site_maneuver_controller ready: "
            f"cmd={self.cmd_vel_topic} pose={self.pose_topic} "
            f"lanelet_pose={self.lanelet_pose_topic} "
            f"auto_start={self.enable_auto_start_from_planning_state} "
            f"entry_mode={self.site_entry_mode} "
            f"adopt_topic={self.adopt_destination_topic} "
            f"cancel_nav2={'true' if self.cancel_nav2_on_site_phase else 'false'}"
        )
    ...
    def _make_pose(self, frame_id: str, x: float, y: float, z: float, yaw_deg: float) -> AvgPoseStamped:
        # HH_260720 - YAML-defined campsite poses are generated internal contracts.
        pose = AvgPoseStamped()
    ...
    def _copy_pose(self, msg: AvgPoseStamped) -> AvgPoseStamped:
        pose = AvgPoseStamped()
    ...
    def _publish_reverse_path(
        ...
        for index in range(steps + 1):
            ...
            # HH_260720 - Standard pose construction is limited to this RViz path boundary.
    ...
    def _stamp_pose_now(self, pose: AvgPoseStamped) -> AvgPoseStamped:
    ...
    def _on_site_goal_ros(self, msg: RosPoseStamped) -> None:
        # HH_260720 - Convert the operator/RViz pose before entering control logic.
        self._on_site_goal(pose_from_ros(msg))

    def _on_site_goal(self, msg: AvgPoseStamped) -> None:
        if self._is_site_internal_phase():
            self._request_nav2_cancel_for_site_phase(force=True)
            self.get_logger().warn(
                "camping_site_maneuver_controller ignored new site_goal while site maneuver is internal: "
        ...
        self.get_logger().info(
            "camping_site_maneuver_controller site_goal updated: "
    ...
    def _on_lanelet_pose(self, msg: AvgPoseStamped) -> None:
        self.lanelet_pose = msg
        self.lanelet_pose_time_s = self._now_s()

    def _mission_key_from_destination(self, msg: UiDestinationCommand) -> str:
        key = str(msg.mission_key).strip()
        if key:
            return key
        site = str(msg.site).strip().upper()
        if site.startswith("B") and site[1:].isdigit():
            return f"{self.site_mission_key_prefix}{int(site[1:])}"
        return site

    def _on_adopt_destination(self, msg: UiDestinationCommand) -> None:
        if not msg.run:
            return
        key = self._mission_key_from_destination(msg)
        if not key:
            self.get_logger().warn("camping_site_maneuver_controller adopt ignored: empty mission key")
            return
        source = str(msg.source).strip() or "adopt_destination"
        self._adopt_wait_return_state(key, source)

    def _pose_usable_as_adopt_route(
        self, route_pose: AvgPoseStamped | None, site_goal: AvgPoseStamped
    ) -> bool:
        if route_pose is None:
            return False
        route_site_distance = self._pose_distance(route_pose, site_goal)
        return self.min_lateral_offset_m <= route_site_distance <= self.max_lateral_offset_m

    def _adopt_wait_return_state(self, key: str, source: str) -> bool:
        # HH_260701 - UI can be selected after a manual goal drove the robot into
        # a campsite. Adopt that parked state so the return button starts the
        # campsite exit instead of being ignored while camping_site_maneuver_controller is IDLE/DONE.
        if self.phase not in {Phase.IDLE, Phase.DONE, Phase.ERROR, Phase.UNLOAD_WAIT, Phase.WAIT_RETURN}:
            self.get_logger().warn(
                f"camping_site_maneuver_controller adopt ignored during active phase={self.phase.value}"
            )
            return False
        if not self._pose_is_fresh():
            self.get_logger().warn("camping_site_maneuver_controller adopt ignored: fresh pose unavailable")
            return False

        site_goal = self.camping_site_goals.get(key)
        if site_goal is None and self.site_goal is not None:
            site_goal = self._copy_pose(self.site_goal)
        if site_goal is None:
            self.get_logger().warn(f"camping_site_maneuver_controller adopt ignored: site goal unavailable for {key}")
            return False
        site_goal = self._stamp_pose_now(self._copy_pose(site_goal))
        distance_to_site = self._pose_distance(self.last_pose, site_goal)  # type: ignore[arg-type]
        if distance_to_site > self.adopt_site_arrival_distance_m:
            self.get_logger().warn(
                "camping_site_maneuver_controller adopt ignored: "
                f"key={key} pose is {distance_to_site:.2f}m from site "
                f"(limit={self.adopt_site_arrival_distance_m:.2f}m)"
            )
            return False

        route_goal_source = "latest_route_goal"
        if self._pose_usable_as_adopt_route(self.route_goal, site_goal):
            route_goal = self._stamp_pose_now(self._copy_pose(self.route_goal))  # type: ignore[arg-type]
        elif self._pose_usable_as_adopt_route(self.lanelet_pose, site_goal):
            route_goal = self._stamp_pose_now(self._copy_pose(self.lanelet_pose))  # type: ignore[arg-type]
            route_goal_source = "latest_lanelet_pose"
        else:
            route_goal = self._stamp_pose_now(self._copy_pose(self.last_pose))  # type: ignore[arg-type]
            route_goal_source = "current_pose_fallback"

        forward, lateral = relative_xy(route_goal, site_goal)
        offset = abs(lateral)
        if offset >= self.min_lateral_offset_m:
            direction = 1.0 if lateral >= 0.0 else -1.0
            source_name = "goal_pair"
        else:
            direction = 1.0 if self.default_lateral_direction != "right" else -1.0
            if route_goal_source in {"latest_route_goal", "latest_lanelet_pose"}:
                offset = min(self._pose_distance(route_goal, site_goal), self.max_lateral_offset_m)
                source_name = f"{route_goal_source}_distance"
            else:
                offset = 0.0
                source_name = "adopt_current_pose"

        self.site_goal = site_goal
        self.route_goal = route_goal
        now_s = self._now_s()
        self.site_goal_time_s = now_s
        self.route_goal_time_s = now_s
        self.site_goal_key = key
        self.last_auto_key = f"adopt:{key}:{now_s:.3f}"

        self.start_yaw = yaw_from_pose(route_goal)
        self.target_yaw = self.start_yaw
        self.rotate_direction_sign = 0.0
        self.rotate_direction_label = "adopted"
        self.crab_direction = direction
        self.return_crab_direction = -direction if self.align_return_yaw_before_crab_out else direction
        self.crab_offset_m = offset
        self.crab_source = source_name
        self.goal_pair_forward_m = forward
        self.entry_start_x = route_goal.pose.position.x
        self.entry_start_y = route_goal.pose.position.y
        self.entry_reference_yaw = self.start_yaw
        self.entry_target_x = site_goal.pose.position.x
        self.entry_target_y = site_goal.pose.position.y
        self.entry_target_yaw = normalize_angle(self.start_yaw + math.pi)
        self.entry_reverse_axis_yaw = self.start_yaw
        effective_crab_speed = max(0.01, self.crab_speed_mps * self.crab_timeout_speed_scale)
        self.crab_duration_s = offset / effective_crab_speed if offset > 0.0 else 0.0
        self.return_requested = False
        self.return_published = False
        self.return_acknowledged = False
        self.last_return_request_publish_s = 0.0
        ...
        self._set_phase(
            Phase.WAIT_RETURN,
            f"adopt={source} key={key} route={route_goal_source} "
            f"dist={distance_to_site:.2f}m offset={offset:.2f}m "
            f"direction={direction:+.0f} source={source_name}",
        )
        return True
    ...
    def _ensure_goal_pair_for_auto_start(self, key: str) -> None:
        ...
        if site_from_config is not None and (
            ...
            self.site_goal_key = key
            self.get_logger().info(
                "camping_site_maneuver_controller restored site_goal from camping_sites_yaml: "
        ...
        if self.route_goal is None and self._pose_is_fresh():
            ...
            self.route_goal_time_s = now_s
            self.get_logger().info(
                "camping_site_maneuver_controller restored route_goal from current pose at Nav2 GOAL_REACHED"
            )
    ...
    def _apply_operation(self, operation: int, source: str) -> tuple[bool, str]:
        # HH_260720 - Route every campsite command through one semantic dispatcher.
        if operation == MotionOperation.START:
            return self._start_sequence(source)
        if operation == MotionOperation.RETURN:
            return self._request_return(source)
        if operation == MotionOperation.CANCEL:
            self._cancel(source)
            return True, "camping-site maneuver cancelled"
        return False, f"unsupported camping-site operation={operation}"
    ...
    def _start_sequence(self, source: str) -> tuple[bool, str]:
        ...
        self.crab_direction = direction
        self.return_crab_direction = direction
        ...
        if source_name == "goal_pair" and abs(forward_residual) > self.max_forward_residual_m:
            ...
            self.get_logger().warn(
                f"camping_site_maneuver_controller goal pair has forward residual {forward_residual:.2f}m. "
    ...
    def _start_reverse_entry_sequence(
        ...
        self.return_crab_direction = self.crab_direction
        self.crab_source = source_name
        self.goal_pair_forward_m = forward_residual
    ...
    def _request_return(self, source: str) -> tuple[bool, str]:
        if self.phase == Phase.WAIT_RETURN:
            ...
            self._begin_return_exit(f"return={source}")
    ...
    def _begin_return_exit(self, reason: str) -> None:
        if self.site_entry_mode == "reverse":
            self._set_phase(Phase.REVERSE_OUT, reason)
            return
        if self.align_return_yaw_before_crab_out:
            self.target_yaw = self.start_yaw
            self.return_crab_direction = -self.crab_direction
            self._set_phase(Phase.ALIGN_RETURN_YAW, f"{reason}; restore_lanelet_yaw")
            return
        self.return_crab_direction = self.crab_direction
        self._set_phase(Phase.CRAB_OUT, reason)
    ...
    def _set_phase(self, phase: Phase, message: str) -> None:
        ...
        self.get_logger().info(f"camping_site_maneuver_controller {phase.value}: {message}")
    ...
    def _publish_amr_service_state_for_phase(self, phase: Phase, source_message: str) -> None:
        ...
        elif phase in {Phase.ALIGN_RETURN_YAW, Phase.REVERSE_OUT, Phase.CRAB_OUT, Phase.DONE}:
        ...
        msg.description = f"camping_site_maneuver_controller:{phase.value}:{source_message}"
        self.amr_service_pub.publish(msg)

    def _publish_zero(self) -> None:
        self.cmd_pub.publish(AvgTwist())

    def _request_nav2_cancel_for_site_phase(self, force: bool = False) -> None:
        if not self.cancel_nav2_on_site_phase or not self._is_site_internal_phase():
            return
        now_s = self._now_s()
        period_s = max(0.1, self.nav2_cancel_period_s)
        if not force and (now_s - self.last_nav2_cancel_request_s) < period_s:
            return
        ...
        sent = False
        for client in self.nav2_cancel_clients:
            if not client.service_is_ready():
                continue
            client.call_async(req)
            sent = True
        if sent:
            self.last_nav2_cancel_request_s = now_s
            if (now_s - self.last_nav2_cancel_log_s) >= 2.0:
                self.last_nav2_cancel_log_s = now_s
                self.get_logger().info(
                    f"camping_site_maneuver_controller requested Nav2 cancel during {self.phase.value}"
                )
    ...
    def _publish_reverse_along_axis(self, target_yaw: float, origin_x: float, origin_y: float) -> None:
        ...
        if self.reverse_entry_debug_period_s > 0.0 and (
            ...
            self.get_logger().info(
                "camping_site_maneuver_controller reverse control: "
    ...
    def _publish_return_request(self, reason: str) -> None:
        ...
        if (
            ...
            return
        msg = PlanningRecallRequest()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.site_name = self.site_goal_key
        msg.source = f"camping_site_maneuver_controller:{reason}"
        self.return_request_pub.publish(msg)
        self.return_published = True
        self.last_return_request_publish_s = now_s
        self.get_logger().info(f"camping_site_maneuver_controller return_to_drop_zone requested: {reason}")

    def _tick(self) -> None:
        elapsed = self._now_s() - self.phase_start_s
        self._request_nav2_cancel_for_site_phase()
        ...
        if self.phase == Phase.ALIGN_ENTRY_YAW:
            ...
            if elapsed >= self.unload_wait_s:
                if self.auto_return_after_unload_wait or self.return_requested:
                    self._begin_return_exit("unload wait complete")
                else:
                    self._set_phase(Phase.WAIT_RETURN, "waiting external return command")
        elif self.phase == Phase.WAIT_RETURN:
            self._publish_zero()
        elif self.phase == Phase.ALIGN_RETURN_YAW:
            if self._publish_rotate():
                self._set_phase(Phase.CRAB_OUT, "return yaw restored")
            ...
            if self._return_reached():
                ...
                # HH_260720 - A yaw-aligned return uses the opposite crab direction;
                # disabling alignment intentionally retains the rotated body frame.
                self._publish_crab(self.return_crab_direction)
            ...
            # HH_260720 - Publishes /control/cmd_vel_raw; idle maneuver zeros can race
    ...
    def _publish_status(self, force: bool = False) -> None:
        ...
        # HH_260720 - Report campsite motion as control telemetry instead of parking telemetry.
        self.status_pub.publish(make_module_state(self, "control", level, message))
...
def main(args=None) -> None:
    ...
    node = CampingSiteManeuverControllerNode()
    try:
        rclpy.spin(node)
