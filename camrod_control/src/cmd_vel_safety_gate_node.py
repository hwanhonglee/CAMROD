...
# HH_260720 - Publish one effective /control/command_enabled state for downstream interlocks.
# HH_260720 - Place final command authorization and shaping under the explicit safety layer.
...
# HH_260720 - Import explicit boundary conversions from the flattened control helper.
from control_support import twist_from_ros, twist_to_ros
...
class CmdVelSafetyGateNode(Node):
    # HH_260720 - Initialize the control-owned cmd_vel safety gate boundary.
    def __init__(self) -> None:
        super().__init__("cmd_vel_safety_gate")

        self.input_topic = str(
            self.declare_parameter("input_topic", "/control/cmd_vel_raw").value
        )
        self.navigation_input_topic = str(
            self.declare_parameter(
                "navigation_input_topic", "/control/nav2_cmd_vel_ros"
            ).value
        )
        self.output_topic = str(
            self.declare_parameter("output_topic", "/control/cmd_vel").value
        )
        self.ros_output_topic = str(
            self.declare_parameter("ros_output_topic", "/control/cmd_vel_ros").value
        )
        ...
        # HH_260720 - Charger departure is keyed by a concrete destination request,
        # not by the mission-engage latch, which may already be true.
        self.mission_request_topic = str(
            self.declare_parameter("mission_request_topic", "/planning/mission_key").value
        )
        self.state_topic = str(
            self.declare_parameter("state_topic", "/control/command_enabled").value
        )
        # HH_260720 - Keep the operator hardware-arm condition at the single final command gate.
        self.platform_drive_enable_topic = str(
            self.declare_parameter(
                "platform_drive_enable_topic", "/platform/drive_enable"
            ).value
        )
        self.require_platform_drive_enable = bool(
            self.declare_parameter("require_platform_drive_enable", True).value
        )
        # HH_260720 - Publish a readable operating state instead of exposing only a AvgBool gate result.
        self.status_topic = str(
            self.declare_parameter(
                "status_topic", "/control/cmd_vel_safety_gate/status"
            ).value
        )
        # HH_260720 - Treat normalized CAN/DBC feedback from camrod_platform as a gate input.
        self.platform_safety_source_mode = str(
            self.declare_parameter(
                "platform_safety_source_mode", "platform_status"
            ).value
        ).strip().lower()
        self.platform_safety_enabled = self.platform_safety_source_mode not in {
            "disabled",
            "off",
            "none",
        }
        ...
        self.platform_status_timeout_s = float(
            self.declare_parameter("platform_status_timeout_s", 0.5).value
        )
        self.block_on_platform_status_stale = bool(
            self.declare_parameter("block_on_platform_status_stale", True).value
        )
        self.block_on_charging = bool(
            self.declare_parameter("block_on_charging", True).value
        )
        # HH_260720 - A new campsite mission may drive off the charger for a bounded release window.
        self.allow_mission_departure_while_charging = bool(
            self.declare_parameter(
                "allow_mission_departure_while_charging", True
            ).value
        )
        self.charging_departure_grace_s = float(
            self.declare_parameter("charging_departure_grace_s", 15.0).value
        )
        self.charger_departure_mission_prefixes = self._parse_source_label_set(
            self.declare_parameter(
                "charger_departure_mission_prefixes", "camping_site_"
            ).value,
            {"camping_site_"},
        )
        self.mission_request_dedup_s = max(
            0.0,
            float(self.declare_parameter("mission_request_dedup_s", 1.0).value),
        )
        self.block_on_platform_error_code = bool(
            self.declare_parameter("block_on_platform_error_code", True).value
        )
        self.require_can_control_mode = bool(
            self.declare_parameter("require_can_control_mode", True).value
        )
        self.critical_battery_stop_enabled = bool(
            self.declare_parameter("critical_battery_stop_enabled", True).value
        )
        self.critical_battery_percentage = float(
            self.declare_parameter("critical_battery_percentage", 0.10).value
        )
        ...
        # HH_260720 - Platform e-stop comes from AvgPlatformStatus; only soft e-stops need topics.
        ...
        self.cost_stop_lookahead_m = float(
            # HH_260720 - Fixed-distance mode is used only when speed-dependent lookahead is disabled.
        ...
        # HH_260622 - AvgPath-based lanelet safety should validate the selected
        ...
        # HH_260720 - Drop-zone exit is control-owned maneuver motion. It can
        # start inside static lanelet/drop-zone boundary cost,
        # so raw lanelet safety must not block it before Nav2 campsite routing.
        self.drop_zone_maneuver_controller_status_topic = str(
            self.declare_parameter(
                "drop_zone_maneuver_controller_status_topic", "/control/drop_zone_maneuver_controller/status"
            ).value
        )
        self.drop_zone_maneuver_controller_static_bypass_phases = self._parse_source_label_set(
            self.declare_parameter(
        ...
        # HH_260720 - Campsite internal motion is owned by camrod_control.
        # Bypass only static lanelet/global-path cost during these phases;
        # live LiDAR/Radar source cost remains blocking.
        self.camping_site_maneuver_controller_status_topic = str(
            self.declare_parameter(
                "camping_site_maneuver_controller_status_topic", "/control/camping_site_maneuver_controller/status"
            ).value
        )
        self.camping_site_maneuver_controller_static_bypass_phases = self._parse_source_label_set(
            self.declare_parameter(
        ...
        # HH_260720 - Fail closed until the operator explicitly arms platform motion.
        self._platform_drive_enabled = False

        # HH_260422: _estop becomes True when any configured estop source publishes True.
        #   True -> blocks cmd_vel regardless of _enabled state; zero AvgTwist is sent immediately.
        self._estop = False
        ...
        # HH_260720 - Cache normalized platform safety inputs used to admit or reject motion.
        self._platform_status_received_sec: float | None = None
        self._platform_vehicle_state = 0
        self._platform_control_mode = 0
        self._platform_error_code = 0
        self._platform_is_charging = False
        self._platform_battery_percentage: float | None = None
        self._charging_departure_override_until_sec = 0.0
        self._last_charger_departure_mission_signature: tuple[str, str, int, int] | None = None
        self._last_charger_departure_mission_time_sec = -1.0e9
        ...
        # HH_260720 - Throttle repeated detections separately from latch-state
        # logging so high-rate cmd_vel callbacks cannot flood operational logs.
        self._last_cost_detection_log_sec = 0.0
        self._last_cost_detection_log_reason = ""
        ...
        self._drop_zone_maneuver_controller_phase = ""
        self._camping_site_maneuver_controller_phase = ""
        ...
        self._last_route_heading_path: AvgPath | None = None
        ...
        self._cost_source_grids: dict[str, AvgOccupancyGrid] = {}
        ...
        self.pub_cmd = self.create_publisher(AvgTwist, self.output_topic, 10)
        # HH_260720 - Ranger/Nav2 standard Twist appears only on explicit `_ros` boundaries.
        self.pub_cmd_ros = self.create_publisher(RosTwist, self.ros_output_topic, 10)
        ...
        self.pub_state = self.create_publisher(AvgBool, self.state_topic, state_qos)
        # HH_260720 - Report why the gate is open or blocked, including charging and CAN faults.
        self.pub_status = self.create_publisher(ModuleState, self.status_topic, state_qos)
        ...
        self.sub_cmd = self.create_subscription(
            AvgTwist, self.input_topic, self._on_cmd, 10
        )
        self.sub_navigation_cmd_ros = self.create_subscription(
            RosTwist,
            self.navigation_input_topic,
            self._on_navigation_cmd_ros,
            10,
        )
        self.sub_engage = self.create_subscription(
            AvgBool, self.engage_topic, self._on_engage, 10
        )
        self.sub_mission_engage = self.create_subscription(
            AvgBool, self.mission_engage_topic, self._on_mission_engage, 10
        )
        self.sub_mission_request = self.create_subscription(
            PlanningMissionKey,
            self.mission_request_topic,
            self._on_mission_request,
            10,
        )
        self.sub_platform_drive_enable = None
        if self.require_platform_drive_enable:
            # HH_260720 - The control gate now owns the former platform interlock input.
            self.sub_platform_drive_enable = self.create_subscription(
                AvgBool,
                self.platform_drive_enable_topic,
                self._on_platform_drive_enable,
                10,
            )
        self.sub_platform_status = None
        if self.platform_safety_enabled:
            # HH_260720 - Consume only platform-normalized CAN data; control does not decode DBC frames.
        ...
        self.sub_drop_zone_maneuver_controller_status = self.create_subscription(
            ModuleState,
            self.drop_zone_maneuver_controller_status_topic,
            self._on_drop_zone_maneuver_controller_status,
            10,
        )
        self.sub_camping_site_maneuver_controller_status = self.create_subscription(
            ModuleState,
            self.camping_site_maneuver_controller_status_topic,
            self._on_camping_site_maneuver_controller_status,
            10,
        )
        ...
        if self.dr_timeout_topic_enabled:
            self.sub_dr_timeout = self.create_subscription(
                AvgBool, self.dr_timeout_topic, self._on_dr_timeout, 10
            )
        ...
        if self.enable_cost_stop:
            ...
            self.sub_cost_grid = self.create_subscription(
                AvgOccupancyGrid, self.cost_grid_topic, self._on_cost_grid, cost_qos
            )
        ...
        if (
            ...
            self.sub_pose = self.create_subscription(
                AvgPoseStamped, self.pose_topic, self._on_pose, 10
            )
            self.sub_odom = self.create_subscription(
                AvgOdometry, self.odometry_topic, self._on_odom, 10
            )
        ...
        self.get_logger().info(
            "cmd_vel_safety_gate ready: "
            f"maneuver_in={self.input_topic} nav2_in={self.navigation_input_topic} "
            f"out={self.output_topic} driver_out={self.ros_output_topic} "
            f"manual_engage_topic={self.engage_topic} "
            f"mission_engage_topic={self.mission_engage_topic} "
            f"soft_estops={','.join(self.additional_estop_topics) or '(none)'} "
            # HH_260720 - Log the platform safety contract used by this gate.
            f"platform_safety={'true' if self.platform_safety_enabled else 'false'} "
            f"platform_status={self.platform_status_topic} "
            ...
            f"site_static_phases={sorted(self.camping_site_maneuver_controller_static_bypass_phases)} "
    ...
    def _on_set_parameters(self, params) -> SetParametersResult:
        ...
        for p in params:
            ...
            elif p.name == "camping_site_maneuver_controller_static_bypass_phases":
                self.camping_site_maneuver_controller_static_bypass_phases = self._parse_source_label_set(
                    p.value,
                    {
                        "align_entry_yaw",
    ...
    # HH_260720 - Convert normalized platform CAN state into explicit motion-block reasons.
    def _platform_block_reasons(self, now_sec: float) -> list[str]:
        if not bool(getattr(self, "platform_safety_enabled", False)):
            return []

        reasons: list[str] = []
        received_sec = getattr(self, "_platform_status_received_sec", None)
        if bool(getattr(self, "block_on_platform_status_stale", True)):
            if received_sec is None:
                reasons.append("platform_status_missing")
            else:
                age_s = max(0.0, now_sec - float(received_sec))
                timeout_s = max(
                    0.0, float(getattr(self, "platform_status_timeout_s", 0.5))
                )
                if timeout_s > 0.0 and age_s > timeout_s:
                    reasons.append(f"platform_status_stale={age_s:.2f}s")

        vehicle_state = int(getattr(self, "_platform_vehicle_state", 0))
        if vehicle_state != 0:
            reasons.append(f"vehicle_state={vehicle_state}")

        control_mode = int(getattr(self, "_platform_control_mode", 0))
        if bool(getattr(self, "require_can_control_mode", True)) and control_mode != 1:
            reasons.append(f"control_mode={control_mode}")

        error_code = int(getattr(self, "_platform_error_code", 0))
        if bool(getattr(self, "block_on_platform_error_code", True)) and error_code != 0:
            reasons.append(
                "platform_error=" + self._format_platform_error_code(error_code)
            )

        if (
            bool(getattr(self, "block_on_charging", True))
            and bool(getattr(self, "_platform_is_charging", False))
            and not self._charging_departure_override_active(now_sec)
        ):
            reasons.append("charging")

        battery_percentage = getattr(self, "_platform_battery_percentage", None)
        if (
            bool(getattr(self, "critical_battery_stop_enabled", True))
            and battery_percentage is not None
            and math.isfinite(float(battery_percentage))
            and float(battery_percentage)
            <= float(getattr(self, "critical_battery_percentage", 0.10))
        ):
            reasons.append(f"critical_battery={100.0 * float(battery_percentage):.1f}%")
        return reasons

    # HH_260720 - Allow only a newly requested mission to release from an active charger.
    def _charging_departure_override_active(self, now_sec: float | None = None) -> bool:
        if not bool(getattr(self, "allow_mission_departure_while_charging", False)):
            return False
        if not bool(getattr(self, "_platform_is_charging", False)):
            return False
        if now_sec is None:
            now_sec = self.get_clock().now().nanoseconds * 1e-9
        return float(
            getattr(self, "_charging_departure_override_until_sec", 0.0)
        ) > float(now_sec)

    # HH_260720 - Build one readable list shared by command admission, logs, and status output.
    def _current_block_reasons(self) -> list[str]:
        now_sec = self.get_clock().now().nanoseconds * 1e-9
        reasons: list[str] = []
        ...
        if self._dr_timeout:
            reasons.append("dr_timeout")
        if bool(getattr(self, "require_platform_drive_enable", False)) and not bool(
            getattr(self, "_platform_drive_enabled", False)
        ):
            # HH_260720 - Report the operator arm independently from planning engage.
            reasons.append("platform_drive_enable=false")
        reasons.extend(self._platform_block_reasons(now_sec))
        if self._cost_stop_latched:
            reasons.append("cost_stop_latched")
        elif self._cost_blocked_until > now_sec:
            reasons.append(f"cost_hold={self._cost_blocked_until - now_sec:.1f}s")
        if self._gnss_recovery_blocked_until > now_sec:
            reasons.append(
                f"gnss_recovery_hold={self._gnss_recovery_blocked_until - now_sec:.1f}s"
            )
        return reasons

    # Returns whether cmd passthrough is currently allowed.
    def _effective_enabled(self) -> bool:
        # HH_260720 - Every reported block reason is authoritative for command admission.
        return not self._current_block_reasons()

    # Publishes current effective engage state and its human-readable operating state.
    def _publish_state(self) -> None:
        ...
        # HH_260720 - Expose CHARGING distinctly while retaining ModuleState severity semantics.
        status_publisher = getattr(self, "pub_status", None)
        if status_publisher is None:
            return
        reasons = self._current_block_reasons()
        status = ModuleState()
        if hasattr(status, "stamp") and hasattr(self.get_clock().now(), "to_msg"):
            status.stamp = self.get_clock().now().to_msg()
        if hasattr(status, "module_name"):
            status.module_name = "cmd_vel_safety_gate"
        charging = "charging" in reasons
        hard_fault = any(
            reason == "estop"
            or reason.startswith("vehicle_state=")
            or reason.startswith("platform_error=")
            or reason.startswith("platform_status_")
        ...
        status.level = (
            ...
            if hard_fault
            else ModuleState.WARN
        )
        departing_charger = enabled and self._charging_departure_override_active()
        operating_state = (
            "DEPARTING_CHARGER"
            if departing_charger
            else "ENABLED"
            if enabled
            else "CHARGING"
            if charging
        ...
        status.message = (
            ...
            f"charging={str(bool(getattr(self, '_platform_is_charging', False))).lower()} "
            f"battery={self._format_battery_percentage()}"
        )
        status_publisher.publish(status)

    # HH_260720 - Format BMS SOC consistently for UI and logs while preserving unknown state.
    def _format_battery_percentage(self) -> str:
        percentage = getattr(self, "_platform_battery_percentage", None)
        if percentage is None or not math.isfinite(float(percentage)):
            return "unknown"
        return f"{100.0 * float(percentage):.1f}%"

    # HH_260720 - Decode the 16-bit ugv_sdk error mask into operator-readable labels.
    @staticmethod
    def _format_platform_error_code(error_code: int) -> str:
        error_bits = (
            (0x0001, "battery_fault"),
            (0x0002, "battery_warning"),
            (0x0004, "remote_signal_lost"),
            (0x0008, "motor1_communication"),
            (0x0010, "motor2_communication"),
            (0x0020, "motor3_communication"),
            (0x0040, "motor4_communication"),
            (0x0080, "steering_encoder"),
            (0x0100, "motor_driver"),
            (0x0200, "upper_controller_communication"),
        )
        labels = [label for mask, label in error_bits if error_code & mask]
        known_mask = sum(mask for mask, _ in error_bits)
        if error_code & ~known_mask:
            labels.append("unknown_bits")
        return f"0x{error_code:04X}({'+'.join(labels)})"

    # HH_260720 - Publish the admitted command through one explicit control output.
    def _publish_command(self, command: AvgTwist) -> None:
        self.pub_cmd.publish(command)
        ros_publisher = getattr(self, "pub_cmd_ros", None)
        if ros_publisher is not None:
            ros_publisher.publish(twist_to_ros(command))

    # Publishes zero AvgTwist when blocked/disabled.
    def _publish_zero(self) -> None:
        self._publish_command(AvgTwist())
    ...
    def _on_navigation_cmd_ros(self, message: RosTwist) -> None:
        # HH_260720 - Convert Nav2's standard command once before safety arbitration.
        self._on_cmd(twist_from_ros(message))

    def _on_cmd(self, msg: AvgTwist) -> None:
        self._last_input_cmd_time = self.get_clock().now()
        self._cmd_input_stale = False
        # HH_260720 - Re-evaluate a latched obstacle on each new command so the
        # continuous-clear timer can release the latch without bypassing safety.
        if self.enable_cost_stop and self._cost_stop_latched:
            self._should_stop_for_cost(msg)
        if self._effective_enabled():
            ...
            if self.enable_cost_stop and self._should_stop_for_cost(cmd_to_publish):
                if self.publish_zero_when_blocked:
                    self._publish_zero()
                return
            self._publish_command(self._scale_twist(cmd_to_publish))
            return
        if self.publish_zero_when_blocked:
            self._publish_zero()
    ...
    def _log_block_reason(self) -> None:
        ...
        # HH_260720 - Keep the log reason identical to the published gate status.
        reasons = self._current_block_reasons()
        self.get_logger().warn(
    ...
    def _set_enabled(self, enabled: bool, source: str = "manual") -> None:
        ...
        if source_key == "mission":
            ...
            # HH_260720 - A concrete mission-key request grants charger departure.
            if not new_latch:
                self._charging_departure_override_until_sec = 0.0
        else:
    ...
    def _on_engage(self, msg: AvgBool) -> None:
    ...
    def _on_mission_request(self, msg: PlanningMissionKey) -> None:
        """Grant a bounded charger departure for a new campsite request."""
        mission_key = self._normalize_source_label(msg.mission_key)
        if not mission_key or not any(
            mission_key.startswith(prefix)
            for prefix in self.charger_departure_mission_prefixes
        ):
            return
        if not bool(getattr(self, "_platform_is_charging", False)):
            return
        ...
        signature = (
            mission_key,
            str(getattr(msg, "source", "")).strip(),
            int(getattr(stamp, "sec", 0)),
            int(getattr(stamp, "nanosec", 0)),
        )
        now_sec = self.get_clock().now().nanoseconds * 1e-9
        if (
            signature == self._last_charger_departure_mission_signature
            and (now_sec - self._last_charger_departure_mission_time_sec)
            < self.mission_request_dedup_s
        ):
            return

        self._last_charger_departure_mission_signature = signature
        self._last_charger_departure_mission_time_sec = now_sec
        self._charging_departure_override_until_sec = (
            now_sec + max(0.0, self.charging_departure_grace_s)
        )
        # HH_260720 - Publish immediately because engage may already be latched true.
        self._publish_state()
        self.get_logger().info(
            f"campsite mission {mission_key} requested while charging: "
            f"allowing charger departure for {self.charging_departure_grace_s:.1f}s"
        )

    # HH_260720 - Track control phases so bounded maneuvers can cross static
    # lanelet/drop-zone cost without disabling live obstacle protection.
    def _on_drop_zone_maneuver_controller_status(self, msg: ModuleState) -> None:
        self._drop_zone_maneuver_controller_phase = self._extract_phase_from_status(msg.message)

    def _on_camping_site_maneuver_controller_status(self, msg: ModuleState) -> None:
        self._camping_site_maneuver_controller_phase = self._extract_phase_from_status(msg.message)

    # HH_260720 - Gate motion from normalized CAN vehicle state, mode, and error mask.
    def _on_platform_status(self, msg: AvgPlatformStatus) -> None:
        previous = (
            self._platform_status_received_sec is None,
            self._platform_vehicle_state,
            self._platform_control_mode,
            self._platform_error_code,
            self._platform_is_charging,
            self._platform_battery_percentage,
            self._estop,
        )
        self._platform_status_received_sec = (
            self.get_clock().now().nanoseconds * 1e-9
        )
        self._platform_vehicle_state = int(msg.vehicle_state)
        self._platform_control_mode = int(msg.control_mode)
        self._platform_error_code = int(msg.error_code)
        self._platform_is_charging = bool(msg.is_charging)
        if not self._platform_is_charging:
            self._charging_departure_override_until_sec = 0.0
        if bool(getattr(msg, "battery_state_available", False)):
            percentage = float(msg.battery_percentage)
            self._platform_battery_percentage = (
                max(0.0, min(1.0, percentage))
                if math.isfinite(percentage)
                else None
            )
        else:
            self._platform_battery_percentage = None
        # HH_260720 - Hardware e-stop and soft e-stops share one fail-closed state.
        self._estop_sources[self.platform_status_topic] = bool(msg.estop)
        self._estop = any(self._estop_sources.values())
        current = (
            False,
            self._platform_vehicle_state,
            self._platform_control_mode,
        ...
        if current != previous:
            ...
            if not self._effective_enabled() and self.publish_zero_when_blocked:
                self._publish_zero()
            self.get_logger().info(
                "platform safety update: "
                f"vehicle_state={self._platform_vehicle_state} "
                f"control_mode={self._platform_control_mode} "
                f"error={self._format_platform_error_code(self._platform_error_code) if self._platform_error_code else 'none'} "
                f"estop={'true' if self._estop else 'false'} "
                f"charging={'true' if self._platform_is_charging else 'false'} "
                f"battery={self._format_battery_percentage()}"
            )

    # HH_260720 - Apply the platform operator arm at the same gate that publishes final cmd_vel.
    def _on_platform_drive_enable(self, msg: AvgBool) -> None:
        drive_enabled = bool(msg.data)
        if drive_enabled == self._platform_drive_enabled:
            return
        self._platform_drive_enabled = drive_enabled
        self._publish_state()
        if not drive_enabled and self.publish_zero_when_blocked:
            self._publish_zero()
        self.get_logger().info(
            "platform drive enable update: "
            f"enabled={'true' if drive_enabled else 'false'} "
            f"effective={'true' if self._effective_enabled() else 'false'}"
        )
    ...
    def _on_dr_timeout(self, msg: AvgBool) -> None:
    ...
    def _on_estop(self, msg: AvgBool, topic: str | None = None) -> None:
        source_topic = str(topic or "soft_estop")
    ...
    def _on_cost_grid(self, msg: AvgOccupancyGrid) -> None:
        self._last_grid = msg
    ...
    def _on_lanelet_safety_grid(self, msg: AvgOccupancyGrid) -> None:
        self._last_lanelet_safety_grid = msg

    # HH_260618: Stores original cost-grid sources for event-time attribution.
    def _on_cost_source_grid(self, label: str, msg: AvgOccupancyGrid) -> None:
    ...
    def _on_route_heading_path(self, msg: AvgPath) -> None:
    ...
    # HH_260720 - Log a changed obstacle cause immediately, then limit repeated
    # reports of that same cause to the configured latch log interval.
    def _log_cost_detection(self, now_sec: float, reason: str, message: str) -> None:
        interval_s = max(0.1, float(self.cost_stop_latch_log_interval_s))
        cause_changed = reason != self._last_cost_detection_log_reason
        interval_elapsed = (
            now_sec - self._last_cost_detection_log_sec
        ) >= interval_s
        if not cause_changed and not interval_elapsed:
            return
        self._last_cost_detection_log_sec = now_sec
        self._last_cost_detection_log_reason = reason
    ...
    def _should_stop_for_cost(self, cmd_in: AvgTwist | None = None) -> bool:
        ...
        if self._should_stop_for_lanelet_safety(cmd_in, now_sec):
            return True
        site_static_bypass = self._should_bypass_static_cost_for_camping_site_maneuver_controller(cmd_in)
        ...
        if self.enable_unavoidable_stop and best_lethal:
            if self._is_unavoidable_cluster(best_lethal, max(1, best_total)):
                reason = f"FRONT:unavoidable:{best_label}"
                self._mark_cost_stop_blocked(now_sec, reason, latch=True)
                self._log_cost_detection(
                    now_sec,
                    reason,
        ...
        for source_label, grid in self._cost_source_grids.items():
            ...
            for pose_label, pose in self._resolve_pose_candidates(target_frame):
                for direction, yaw_off, lookahead, width, threshold, _ in corridors:
                    if direction == "FRONT" and self.front_dynamic_stop_use_local_path:
                        ...
                        if path_valid:
                            if not path_blocked:
                                ...
                                continue
                            wx, wy, cost, _ = path_detail
                            reason = f"FRONT_PATH:{pose_label}:{source_label}:{cost}"
                            self._mark_cost_stop_blocked(now_sec, reason, latch=True)
                            self._log_cost_detection(
                                now_sec,
                                reason,
                    ...
                    reason = f"{direction}:{pose_label}:{source_label}:{cost}"
                    self._mark_cost_stop_blocked(now_sec, reason, latch=True)
                    self._log_cost_detection(
                        now_sec,
                        reason,
    ...
    def _should_bypass_static_cost_for_camping_site_maneuver_controller(
        self, cmd_in: AvgTwist | None
    ) -> bool:
        if cmd_in is None:
            return False
        ...
        for label, grid in self._cost_source_grids.items():
            ...
            for pose_label, pose in self._resolve_pose_candidates(target_frame):
                ...
                if not blocked:
                    continue
                wx, wy, cost = detail
                reason = f"ROTATE:{pose_label}:{label}:{cost}"
                self._mark_cost_stop_blocked(now_sec, reason, latch=True)
                self._log_cost_detection(
                    now_sec,
                    reason,
    ...
    def _is_translational_cmd(self, cmd_in: AvgTwist) -> bool:
    ...
    def _should_stop_for_lanelet_safety(
        self, cmd_in: AvgTwist | None, now_sec: float
    ) -> bool:
        ...
        if self._should_bypass_static_lanelet_for_drop_zone_exit(cmd_in, now_sec):
            return False
        if self._should_bypass_static_lanelet_for_camping_site_maneuver_controller(cmd_in, now_sec):
            return False
    ...
    def _extract_phase_from_status(self, message: str) -> str:
        # HH_260720 - Maneuver ModuleState.message uses "phase=<PHASE> ...".
    ...
    def _should_bypass_static_lanelet_for_drop_zone_exit(
        self, cmd_in: AvgTwist, now_sec: float
    ) -> bool:
        ...
        if (now_sec - self._last_drop_zone_static_bypass_log_sec) >= 1.0:
            ...
            self.get_logger().warn(
                ...
                f"phase={self._drop_zone_maneuver_controller_phase} "
                f"cmd_x={float(cmd_in.linear.x):.2f}m/s"
            )
        return True

    def _should_bypass_static_lanelet_for_camping_site_maneuver_controller(
        self, cmd_in: AvgTwist, now_sec: float
    ) -> bool:
        ...
        if (now_sec - self._last_site_static_phase_bypass_log_sec) >= 1.0:
            ...
            self.get_logger().warn(
                ...
                f"phase={self._camping_site_maneuver_controller_phase} "
                f"cmd_x={float(cmd_in.linear.x):.2f}m/s "
                f"cmd_y={float(cmd_in.linear.y):.2f}m/s"
            )
        return True

    def _log_site_lanelet_static_bypass(self, cmd_in: AvgTwist, now_sec: float) -> None:
        if (now_sec - self._last_lateral_static_bypass_log_sec) < 2.0:
            return
        self._last_lateral_static_bypass_log_sec = now_sec
        self.get_logger().info(
    ...
    def _is_drop_zone_exit_phase(self) -> bool:
        # HH_260720 - Keep drop-zone exit checks centralized so route heading
        # and static lanelet safety agree on control-owned motion.
        phase = self._normalize_source_label(self._drop_zone_maneuver_controller_phase)
        return bool(phase and phase in self.drop_zone_maneuver_controller_static_bypass_phases)

    def _is_site_static_bypass_phase(self) -> bool:
        phase = self._normalize_source_label(self._camping_site_maneuver_controller_phase)
        return bool(phase and phase in self.camping_site_maneuver_controller_static_bypass_phases)
    ...
    def _apply_yaw_alignment_gate(self, cmd_in: AvgTwist) -> AvgTwist | None:
    ...
    def _apply_route_heading_alignment_gate(self, cmd_in: AvgTwist) -> AvgTwist | None:
        if not self.enable_route_heading_alignment:
            self._route_heading_align_active = False
            return None
        if self._is_drop_zone_exit_phase():
            # HH_260720 - Drop-zone control owns exit/alignment before a Nav2 path exists.
            self._route_heading_align_active = False
            return None
        if self._is_site_static_bypass_phase():
            # HH_260720 - During campsite entry/return, camrod_control owns
    ...
    def _route_heading_from_path(
        self, path: AvgPath, pose_x: float, pose_y: float, pose_yaw: float
    ...
    # HH_260619 - AvgPath-based lanelet safety sampler for forward Nav2 driving.
    ...
    # HH_260702 - AvgPath-based dynamic obstacle sampler for forward avoidance.
    ...
    def _sample_grid_cost_detail(
        self, grid: AvgOccupancyGrid, wx: float, wy: float
    ) -> tuple[bool, int]:
        res = float(grid.info.resolution)
        if res <= 0.0:
            return False, -1
    ...
    def _sample_grid_cost(self, grid: AvgOccupancyGrid, wx: float, wy: float) -> int:
        res = float(grid.info.resolution)
        if res <= 0.0:
            return -1
        origin_x = float(grid.info.origin.position.x)
    ...
    def _scale_twist(self, msg: AvgTwist) -> AvgTwist:
...
def main(args=None) -> None:
    ...
    try:
        ...
        # HH_260720 - A second launch SIGINT can arrive while rclpy is
        # destroying publishers; treat that teardown race as a clean stop.
        try:
            node.destroy_node()
