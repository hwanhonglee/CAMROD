...
| Body safety | Physical `1.39160 x 1.07000 m` stops ordinary motion; only proven monotonic inward virtual-boundary escape is eligible |
| Planning margin | `1.59160 x 1.27000 m`, 10 cm per side; margin-only contact can request bounded recovery |
...
| Speed | Final cruise `2.0 km/h`; maneuver ratios are listed below; recovery remains `0.18 km/h` |
| Pose/control clock | Real EKF, local path/tracking and Nav2 controller use `20 Hz`; delayed-data history is `1.0 s` |
| GNSS correction clock | HH_260807 hardware launch writes `5 Hz` to match 200 ms moving-base epochs; physical link acceptance remains pending |
...
| Runtime | Checker/Nav2/system-core containers ON; LiDAR cost grid and DDS-SHM OFF by default; WebKit is the default operator window |
...
|---|---:|
| RPP cruise | `2.000 km/h` |
| Curve floor / final approach | `1.000 / 0.500 km/h` |
| Campsite crab | `1.200 km/h` |
| Campsite reverse / drop-zone exit / reverse parking | `0.800 km/h` |
| AprilTag approach / insertion | `1.000 / 0.250 km/h` |
| Optional yaw-zone approach 1 / 2 | `1.250 / 1.000 km/h` |
...
| `4`, `11`, `12` footprint/recovery | `P0-D` | Real margin/body escape proof, retry budget and mission continuation |
...
| `10`, `13` GNSS/IMU/control latency | `P0-B` | Fixed reacquisition, 5 Hz active/1 Hz comparison and 2 km/h lateral convergence |
...
3. Drive straight at `1`, then `2 km/h` with one common localization/control bag.
4. Exercise one-side planning-margin recovery and a virtual physical-body inward-escape proof; actual obstacles remain stopped.
...
At the current `2 km/h`, the robot moves `0.1111 m` between configured 5 Hz fixes
and about `0.30 m` in `0.54 s`. A normal one-epoch delta stays below the proposed gate,
but missed epochs or an unaligned latest-pose comparison can exceed it. Use the
timestamp-aligned EKF innovation rather than a previous-fix displacement gate.
