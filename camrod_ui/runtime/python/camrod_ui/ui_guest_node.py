...
class UiGuestNode(Node):
    ...
    def _on_service_state(self, msg: AvgServiceState) -> None:
        ...
        with self._lock:
            ...
            # Service state is a lifecycle observation, not mission-identity
    ...
    def _on_destination_dispatch_status(self, msg: String) -> None:
        ...
        with self._lock:
            ...
            if (
                ...
                return
            previous_final_ready = getattr(self, "_battery_parking_policy", {}).get("recall_final_return_ready", False)
            self._battery_parking_policy = guest_battery_parking_policy_payload(
                payload, getattr(self, "_battery_parking_policy", None)
            )
            if (previous_final_ready != self._battery_parking_policy["recall_final_return_ready"]
            ...
            if has_service_state:
                ...
                # HH_260907 - Same service ID can advance through clearance,
                # site entry, rotation, and exit. Persist it for reconnect too.
    ...
    def _reserve_guest_return_request(
        ...
        with self._lock:
            ...
            # A delayed first-stage click must never approve departure after
            # rotation. Require an actual JSON bool matching backend readiness.
            ...
            try:
                ...
                while True:
                    ...
                    # (확장) 이용 완료 → 드롭존 복귀
                        ...
                        recall_final_return = payload.get("recall_final_return", False)
                        admitted, error, current = await asyncio.to_thread(
                            node._reserve_guest_return_request, recall_final_return
                        )
                        if admitted:
                            try:
                                await asyncio.to_thread(node._publish_usage_complete, recall_final_return)
                            except Exception:
                                with node._lock:
